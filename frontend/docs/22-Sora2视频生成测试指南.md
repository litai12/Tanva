...
| 问题 | 解决方案 |
|------|--------|
| 页面不显示 | 确保使用 `?sora2-test=true` 参数 |
| API 密钥错误 | 检查是否正确复制，确保以 `sk-` 开头 |
| 响应超时 | 大型生成可能需要较长时间，请耐心等待 |
| 流式响应不工作 | 检查浏览器是否支持 ReadableStream API（现代浏览器都支持） |

## 调试技巧

1. 打开浏览器控制台 (F12 或 Cmd+Option+I)
2. 查看 Console 选项卡查看详细日志：
   - ✅ 成功消息（绿色）
   - ⚠️ 警告消息（黄色）
   - ❌ 错误消息（红色）
3. 检查 Network 标签查看 API 请求和响应
