...
- CORS：来源取自 `CORS_ORIGIN`（逗号分隔），`credentials: true`；`CORS_ORIGIN=*` 可放开所有来源（仅建议本地/测试）。
...
- 登录提醒：
  - 管理端通过 `/api/admin/settings` 保存系统设置 `login_notice`，值为 JSON 字符串；支持 `{ "enabled": boolean, "content": string, "contentHtml": string, "mediaType": "image" | "video" | null, "mediaUrl": string, "posterUrl": string, "primaryButtonText": string, "primaryButtonUrl": string, "secondaryButtonText": string, "secondaryButtonUrl": string }`。
  - `contentHtml` 为受限富文本，`content` 为兼容用纯文本；媒体 URL 只允许远程 URL 或站内路径。
  - 用户登录后可读取 `GET /api/settings/login-notice`；后端仅在开启且内容非空时返回 `enabled: true`。
...
- 接口（均需登录）：
  ...
  - 约束：`contentJson` 属于「设计 JSON」，只允许保存远程 URL/路径引用；禁止 `data:`/`blob:`/裸 base64（例如 `iVBORw0...`）进入 DB/OSS（后端会强制清洗）。
