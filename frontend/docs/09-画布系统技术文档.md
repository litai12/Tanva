...
```typescript
// PaperCanvasManager.tsx + utils/canvasViewportFrame.ts
...
`PaperCanvasManager` 与 `FlowOverlay` 共享同一个 `canvasViewportFrame`
RAF 快照，保证 Paper 图片层和 ReactFlow 节点层使用同一份
`zoom/pan/dpr` 计算结果，避免缩放时出现一帧错位。
...
3. **缩放自适应**: `zoom >= 0.4` 显示主/副网格；`0.3 <= zoom < 0.4` 只显示主网格并覆盖完整视口；`zoom < 0.3` 隐藏网格线

```typescript
// Canvas.tsx:86-99
const showGridLines = zoom >= 0.3;          // 网格线显示阈值
const showMinorGrid = zoom >= 0.4;          // 副网格显示阈值
