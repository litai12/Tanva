...
## 2D 转 3D 生成链路（2026-07-22）

图片工具栏的“2D转3D”使用腾讯混元 3D 3.1，前端不再用一个 HTTP 长连接等待完整生成。`convert2Dto3DService.ts` 先调用 `POST /api/ai/convert-2d-to-3d-async` 获取 `taskId`，再每 3 秒调用 `GET /api/ai/convert-2d-to-3d/task/:taskId`，最长持续 15 分钟；成功后仍通过 `canvas:insert-model3d` 插入远程模型。

每次用户操作都会生成稳定的 `clientRequestId`，并按项目与图片 ID 暂存在 `sessionStorage`。网络中断、组件重建或轮询超时后，再次点击会提交相同请求身份并恢复原任务；明确成功或失败后才清理。后端将 `userId + taskType + clientRequestId` 哈希为确定性 `VideoTask.id`，数据库主键保证并发重复提交只有一个后台任务和一次积分扣除。

状态查询必须通过当前登录用户校验 `VideoTask.userId`。查询以持久化任务为兜底，因此内存状态过期后仍可返回已完成的 OSS 模型 URL。账单与管理端外显使用真实的 `provider=hunyuan-3d`、`model=3.1`；该生成链路不经过 GPT/new-api。
