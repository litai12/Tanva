...
- 关键点：
  ...
  - 登录后进入受保护路由时会读取 `GET /api/settings/login-notice`；若后台 `login_notice` 已开启且内容非空，则展示需要用户手动关闭的提醒弹窗，公开首页不触发。`login_notice` 支持顶部图片/视频媒体、底部富文本和主/次按钮配置；前端会在渲染富文本前做白名单清洗，并拒绝 `data:`/`blob:`/`javascript:` 媒体 URL。
...
- `GET /api/auth/watcha/authorize?returnTo=/app`
...
观猹 OAuth（新增）：
- 登录页在“登录”按钮下方新增观猹入口图标按钮。
- 点击后前端跳转 `/api/auth/watcha/authorize`，后端生成 `state` 并重定向到观猹授权页。
- 授权成功后观猹回调 `/api/auth/watcha/callback`，后端完成 `code -> token -> userinfo` 并签发本站 cookie，最终回跳前端 `returnTo`（默认 `/app`）。
- 新增服务端配置：`WATCHA_OAUTH_CLIENT_ID`、`WATCHA_OAUTH_CLIENT_SECRET`、`WATCHA_OAUTH_REDIRECT_URI`（必填），可选 `WATCHA_OAUTH_SCOPE`、`WATCHA_OAUTH_FRONTEND_BASE_URL`、`WATCHA_OAUTH_FAILURE_PATH`。
...
- 约束：项目 `contentJson` 为设计 JSON，只允许保存远程 URL/路径引用；禁止 `data:`/`blob:`/base64 图片等内联内容进入 DB/OSS。
