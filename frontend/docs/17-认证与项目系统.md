...
- 关键点：
  ...
  - 登录后进入受保护路由时会读取 `GET /api/settings/login-notice`；若后台 `login_notice` 已开启且内容非空，则展示需要用户手动关闭的提醒弹窗，公开首页不触发。`login_notice` 支持 `{ enabled, content, contentHtml }`，前端会在渲染富文本前做白名单清洗。
