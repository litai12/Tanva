...
- 自动保存钩子：`src/hooks/useProjectAutosave.ts`
  ...
  - 仅当 `content.flow.nodes` 至少包含 1 个节点时参与自动保存；Flow 缺失或节点为空时保留本地 `dirty` 状态，不发起云端保存请求
...
- 手动保存（按钮或 `Ctrl/Cmd + S`）会在 `PUT /api/projects/:id/content` 中携带 `createWorkflowHistory=true`，后端仅将 `content.flow` 写入 `WorkflowHistory`（不存整份超大内容快照）。
- 右上角「工作流历史」图标：悬浮/点击打开浮层，可查看历史版本列表并支持“恢复并保存”。
- 相关接口：
  - `GET /api/projects/:id/workflow-history?limit=50`
  - `GET /api/projects/:id/workflow-history/:updatedAt`（`updatedAt` 为 ISO 字符串，需要 URL encode）
...
4) 定时或条件触发自动保存；若 Flow 节点为空则跳过，否则合并 `paperJson` → `PUT /api/projects/:id/content`
5) 服务端返回新的 `version` 与 `updatedAt`，前端清除 `dirty` 并更新状态

## 注意事项

- 切换项目会清空跨项目缓存（如 AI 上下文图片缓存），避免“继承隐藏状态”。
- 空 Flow 过滤只作用于自动保存。用户明确通过保存按钮、`Ctrl/Cmd + S` 或历史恢复触发的手动保存仍使用独立链路。
