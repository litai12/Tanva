...
- `thumbnail`：相对路径或远程 URL（可选；**禁止 dataURL/base64**）。
...
```json
{
  ...
  "description": "可选介绍",
  "tags": ["basic"],
  "thumbnail": "可选，相对路径或远程 URL（禁止 dataURL/base64）",
  "nodes": [ /* 节点数组 */ ],
  "edges": [ /* 连线数组 */ ]
}
```
...
- 图片类字段：
  - **只允许**保存稳定的远程 URL/路径引用（例如 OSS URL），如 `data.imageUrl` / `data.imageUrls[]`。
  - **禁止**把 `data:` / `blob:` / 裸 base64（例如 `iVBORw0...`）写入模板 JSON（包括 `thumbnail`、`imageData`、`images[]` 等字段）；后端/持久化层会强制清洗并移除这些内联内容。
  - 建议使用 Flow 面板的「导出模板」功能：会自动将 `flow-asset:`/`blob:`/`data:image/*`/裸 base64 等临时态图片引用上传并替换为可持久化的 OSS key/URL，避免公共模板图片丢失。
...
```json
{
  ...
  "nodes": [
    ...
    { "id": "img1", "type": "image",      "position": { "x": 620, "y": -10 }, "data": { "imageUrl": null },            "boxW": 260, "boxH": 240 }
  ],
  "edges": [
