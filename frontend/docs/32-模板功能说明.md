...
- 图片类字段：
  - 推荐：只保存稳定的远程 URL（例如 OSS URL），如 `data.imageUrl` / `data.imageUrls[]`，避免把大体积 base64 写进模板 JSON。
  - 预览用：可保留 `thumbnail`（dataURL，通常为压缩后的 JPEG）以提升加载/渲染体验。
  - 兼容：`imageData` / `images[]` 仍可用，但体积大时不建议写入模板文件。
