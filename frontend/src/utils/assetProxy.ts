...
function getApiBaseUrl(): string {
  ...
  const raw =
    ...
    (typeof viteEnv.VITE_API_URL === 'string' && viteEnv.VITE_API_URL) ||
    (typeof viteEnv.VITE_API_BASE === 'string' && viteEnv.VITE_API_BASE) ||
    '';
...
export function proxifyRemoteAssetUrl(input: string): string {
  ...
  if (value.startsWith('/api/assets/proxy') || value.startsWith('/assets/proxy')) {
