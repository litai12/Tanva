...
test("prompt optimizer exposes only DeepSeek", () => {
  ...
  assert.equal(DEFAULT_PROMPT_OPTIMIZATION_MODEL, "deepseek-v4.1-flash");
});

test("legacy, unavailable and unknown prompt optimizer models migrate to DeepSeek", () => {
  assert.equal(resolvePromptOptimizationModel("gpt-5.4"), "deepseek-v4.1-flash");
  assert.equal(resolvePromptOptimizationModel(undefined), "deepseek-v4.1-flash");
  assert.equal(resolvePromptOptimizationModel("deepseek-v4-flash"), "deepseek-v4.1-flash");
  assert.equal(
