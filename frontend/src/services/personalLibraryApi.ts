...
import type { PersonalLibraryAsset, PersonalAssetType } from '@/stores/personalLibraryStore';
...
export const personalLibraryApi = {
  ...
  async patch(id: string, patch: Partial<PersonalLibraryAsset>): Promise<PersonalLibraryAsset> {
    const res = await fetchWithAuth(`${base}/api/personal-library/assets/${encodeURIComponent(id)}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
