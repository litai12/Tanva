...
function isUploadTimeoutError(message?: string): boolean {
        ...
        if (imageUploadWorkerClient.isSupported()) {
          try {
            ...
            if (!workerResult.success) {
              if (isUploadTimeoutError(workerResult.error)) {
                return {
                  success: false,
                  error: workerResult.error || "图片上传超时，请重试",
                };
              }
            ...
            if (isUploadTimeoutError(message)) {
              return {
                success: false,
                error: message || "图片上传超时，请重试",
              };
            }
