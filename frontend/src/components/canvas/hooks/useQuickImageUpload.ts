...
const isBase64String = (value?: string | null): value is string => {
  if (typeof value !== "string") return false;
  // 检查是否是base64字符集（不包含特殊字符如/+:等，这些在data URL中会出现）
  // 同时排除已经是完整data URL的情况
  if (
  ...
  // base64通常只包含A-Z、a-z、0-9、+、/、=字符
  const base64Regex = /^[A-Za-z0-9+/]*={0,2}$/;
  // 移除可能的空白字符后检查
  const trimmed = value.trim();
  // 长度合理且符合base64格式
  return trimmed.length > 10 && base64Regex.test(trimmed);
};
...
const pickRasterSource = (
  asset: StoredImageAsset
): { source: string; remoteUrl?: string } => {
  const remoteUrl = isRemoteUrl(asset.url)
    ? asset.url
    : isRemoteUrl(asset.src)
    ? asset.src
    : undefined;
  if (remoteUrl) {
  ...
  // 检查内联数据URL（base64或blob）
  const inline = isInlineDataUrl(asset.localDataUrl)
    ? asset.localDataUrl
    : isInlineDataUrl(asset.src)
    ? asset.src
    : undefined;

  if (inline) {
    return { source: inline, remoteUrl: undefined };
  }

  // 检查纯base64字符串（兼容旧数据）
  const base64Source = isBase64String(asset.localDataUrl)
    ? `data:image/png;base64,${asset.localDataUrl}`
    : isBase64String(asset.src)
    ? `data:image/png;base64,${asset.src}`
    : isBase64String(asset.url)
    ? `data:image/png;base64,${asset.url}`
    : undefined;

  if (base64Source) {
    return { source: base64Source, remoteUrl: undefined };
  }

  // 最后的回退
  return { source: asset.url || asset.src || "", remoteUrl: undefined };
};
...
export const useQuickImageUpload = ({
  context,
  canvasRef,
  projectId,
}: UseQuickImageUploadProps) => {
  ...
  const parallelGroupCollectorRef = useRef<
    Map<string, { total: number; imageIds: string[] }>
  >(new Map());
  ...
  const removePredictedPlaceholder = useCallback(
    (placeholderId: string | undefined | null) => {
  ...
  const updatePlaceholderProgress = useCallback(
    (placeholderId: string, progress: number) => {
      if (!placeholderId) return;
      const existing = predictedPlaceholdersRef.current.get(placeholderId);
      if (!existing || !existing.parent) return;

      // 查找进度标签并更新 - 使用索引而不是直接引用
      const progressLabelIndex = existing.data?.progressLabelIndex as
        | number
        | undefined;
      const progressLabel =
        progressLabelIndex !== undefined && existing.children
          ? (existing.children[progressLabelIndex] as
              | paper.PointText
              | undefined)
          : undefined;
  ...
  const generationLayoutRef = useRef<{
    ...
    rowAssignments: Map<
      string,
      { rowIndex: number; rowSpan: number; columns: number }
    >;
  }>({
    baseAnchor: null,
    nextRow: 0,
  ...
  const allocateRowForBatch = useCallback(
    (batchKey: string, columns: number, rowsNeeded: number) => {
        ...
        for (const item of layer.children) {
          ...
          if (
            ...
            if (item instanceof paper.Group) {
              // 从组中找到Raster对象
              raster = item.children.find((child) =>
                isRaster(child)
              ) as paper.Raster;
  ...
  const calculateSmartPosition = useCallback(
    (
      ...
      const getSpacingHorizontal = () =>
        useUIStore.getState().smartPlacementOffsetHorizontal || 522;
      const getSpacingVertical = () =>
        useUIStore.getState().smartPlacementOffsetVertical || 552;
      ...
      switch (operationType) {
        case "generate": {
          ...
          if (currentImageId && currentImageId.startsWith("ai-placeholder-")) {
            const placeholder =
              predictedPlaceholdersRef.current.get(currentImageId);
          ...
          const columns =
            groupTotal > 1 ? Math.min(4, Math.max(1, groupTotal)) : 1;
          const rowsNeeded = Math.max(1, Math.ceil(groupTotal / columns));
          const batchKey =
            groupId ||
            currentImageId ||
            `generate-${existingImages.length}-${Date.now()}`;
          ...
          if (!generationLayoutRef.current.baseAnchor) {
            ...
            if (
              ...
              generationLayoutRef.current.baseAnchor = {
                x: anchor.x,
                y: anchor.y,
              };
            } else {
          ...
          const anchor = generationLayoutRef.current.baseAnchor ?? {
            x: viewCenter.x,
            y: viewCenter.y,
          };
          ...
          const rowIndex =
            assignment.rowIndex + Math.floor(groupIndex / columns);
          const colIndex = Math.min(
            columns - 1,
            Math.max(0, groupIndex % columns)
          );
        ...
        case "blend": {
          ...
          if (sourceImages && sourceImages.length > 0) {
            ...
            if (firstSourceImage) {
              const position = {
                x: firstSourceImage.x + spacingH,
                y: firstSourceImage.y,
              };
              return position;
            }
          }
  ...
  const showPredictedPlaceholder = useCallback(
    (params: {
      placeholderId: string;
      ...
      if (!paper.project || !paper.view) {
        ...
        if (retries > 0) {
          setTimeout(
            () => showPredictedPlaceholder({ ...params, retries: retries - 1 }),
            180
          );
        }
        return;
      ...
      const preferHorizontal =
        params.preferHorizontal || (params.groupTotal ?? 1) > 1;
      const layoutContext = {
        groupId: params.groupId,
        groupIndex: params.groupIndex,
        groupTotal: params.groupTotal,
        anchorCenter:
          params.groupAnchor ?? params.center ?? params.smartPosition ?? null,
        preferHorizontal,
      };
      ...
      group.children?.forEach((child: paper.Item) =>
        attachPlaceholderMeta(child)
      );
      ...
      const animate = () => {
        ...
        const shimmerProgress =
          (animationTime % animationDuration) / animationDuration; // 0-1 循环
        ...
        shimmer.position = new paper.Point(
          currentX + shimmerWidth / 2,
          centerPoint.y
        );
      ...
      try {
        ...
        if (!isInView) {
          ...
          logger.debug(
            `🎯 自动聚焦视角到占位框: (${centerPoint.x.toFixed(
              1
  ...
  const findImagePlaceholder = useCallback((placeholderId?: string) => {
    try {
      if (placeholderId) {
        ...
        if (existing) {
          logger.upload(
            `✅ [findImagePlaceholder] 从 predictedPlaceholdersRef 找到占位符: ${placeholderId}`
          );
          return existing;
        }
      }
      ...
      if (!paper.project) {
        logger.upload(
          `⚠️ [findImagePlaceholder] Paper.js 项目未初始化，placeholderId: ${placeholderId}`
        );
        return null;
      }
        ...
        for (const item of layer.children) {
          if (item.data?.type === "image-placeholder" && item.data?.bounds) {
            if (!placeholderId || item.data?.placeholderId === placeholderId) {
              logger.upload(
                `✅ [findImagePlaceholder] 从图层中找到占位符: ${
                  placeholderId || "any"
                }`
              );
              return item;
      ...
      if (placeholderId) {
        logger.upload(
          `⚠️ [findImagePlaceholder] 未找到占位符: ${placeholderId}，当前占位符数量: ${predictedPlaceholdersRef.current.size}`
        );
      }
      return null;
    } catch (error) {
  ...
  const handleQuickImageUploaded = useCallback(
    async (
      ...
      if (typeof imagePayload === "string") {
        // 🔥 检查图片类型：远程 URL、base64 data URL、还是纯 base64 字符串
        const isRemoteUrl =
          imagePayload.startsWith("http://") ||
          imagePayload.startsWith("https://");
        const isDataUrl = imagePayload.startsWith("data:image/");
        const isBlobUrl = imagePayload.startsWith("blob:");

        if (isRemoteUrl) {
          // 如果是远程 URL，直接使用，不需要上传
          logger.upload(
            `🌐 [handleQuickImageUploaded] 检测到远程 URL，直接使用: ${imagePayload.substring(
              0,
              50
            )}...`
          );
          asset = {
            id: `remote_img_${Date.now()}_${Math.random()
              .toString(36)
              .slice(2, 8)}`,
            url: imagePayload,
            src: imagePayload,
          ...
          // 如果是 base64 data URL 或 blob URL
          if (skipUpload) {
          ...
          // 如果是纯 base64 字符串（没有 data:image/ 前缀），先转换为 data URL 格式
          const dataUrl = `data:image/png;base64,${imagePayload}`;
          logger.upload(
            `🔄 [handleQuickImageUploaded] 检测到纯 base64 字符串，已转换为 data URL: ${dataUrl.substring(
              0,
              50
            )}...`
          );
          ...
          if (skipUpload) {
            const resolvedName = fileName || "base64-image.png";
            asset = {
              id: `local_img_${Date.now()}_${Math.random()
                .toString(36)
                .slice(2, 8)}`,
        ...
        // imagePayload is StoredImageAsset here (type narrowing from the if condition)
        const assetObj = imagePayload as unknown as StoredImageAsset;
        asset = {
          ...assetObj,
          src: assetObj.url || assetObj.src,
          localDataUrl: isInlineDataUrl(assetObj.localDataUrl)
            ? assetObj.localDataUrl
            : isInlineDataUrl(assetObj.src)
            ? assetObj.src
            : undefined,
        };
        fileName = asset.fileName || fileName;
      ...
      const { source: rasterSource, remoteUrl: resolvedRemoteUrl } =
        pickRasterSource(asset);
      try {
        ensureDrawingLayer();
        ...
        if (!placeholder && placeholderId) {
          const placeholderFromRef =
            predictedPlaceholdersRef.current.get(placeholderId);
          if (placeholderFromRef) {
            placeholder = placeholderFromRef;
            logger.upload(
              `🎯 从 predictedPlaceholdersRef 找到占位符: ${placeholderId}`
            );
          }
        }
        ...
        const imageId =
          placeholderId ||
          asset.id ||
          `quick_image_${Date.now()}_${Math.random()
            .toString(36)
            .substr(2, 9)}`;
        ...
        if (smartPosition) {
          const desiredPoint = new paper.Point(
            smartPosition.x,
            smartPosition.y
          );
          pendingEntry = registerPending(desiredPoint);
          const adjustedPoint = findNonOverlappingPosition(
            desiredPoint,
            baseWidth,
            baseHeight,
          ...
          if (!desiredPoint.equals(adjustedPoint)) {
            logger.upload(
              `📍 快速上传：智能位置冲突，已调整至 (${adjustedPoint.x}, ${adjustedPoint.y})`
            );
          } else {
            logger.upload(
              `📍 快速上传：使用智能位置 (${adjustedPoint.x}, ${adjustedPoint.y})`
            );
          }
          ...
          logger.upload(
            `📍 快速上传：使用占位符原位置 (${placeholderCenter.x.toFixed(
              1
            )}, ${placeholderCenter.y.toFixed(1)})`
          );
        } else if (operationType) {
          pendingEntry = registerPending(null);
          const calculated = calculateSmartPosition(
            operationType,
            sourceImageId,
            sourceImages,
            imageId
          ...
          if (!desiredPoint.equals(adjustedPoint)) {
            logger.upload(
              `📍 快速上传：智能计算位置 (${desiredPoint.x}, ${desiredPoint.y}) → 调整为 (${adjustedPoint.x}, ${adjustedPoint.y}) 操作类型: ${operationType}`
            );
          } else {
            logger.upload(
              `📍 快速上传：计算智能位置 (${adjustedPoint.x}, ${adjustedPoint.y}) 操作类型: ${operationType}`
            );
          }
        } else {
          const centerSource =
            paper.view && (paper.view as any).center
          ...
          if (!centerPoint.equals(adjustedPoint)) {
            logger.upload(
              `📍 快速上传：视口中心冲突，已调整至 (${adjustedPoint.x.toFixed(
                1
              )}, ${adjustedPoint.y.toFixed(1)})`
            );
          } else {
            logger.upload(
              `📍 快速上传：默认使用视口中心 (${adjustedPoint.x.toFixed(
                1
              )}, ${adjustedPoint.y.toFixed(1)})`
            );
          }
        ...
        raster.onLoad = () => {
          ...
          const alreadyInitialized = Boolean(
            (raster as any)?.data?.__tanvaImageInitialized
          );
          ...
          const useOriginalSize =
            localStorage.getItem("tanva-use-original-size") === "true";
          ...
          if (placeholderId) {
            ...
            if (placeholder && placeholder.data?.bounds) {
              ...
              if (placeholderFromRef && placeholderFromRef.data?.bounds) {
                ...
                boundsSource = "placeholder";
                logger.upload(
                  `✅ [raster.onLoad] 从 predictedPlaceholdersRef 找到占位符: ${placeholderId}`,
                  targetBounds
                );
                logger.upload(
                  `🎯 从 predictedPlaceholdersRef 找到占位符: ${placeholderId}`
                ...
                logger.upload(
                  `⚠️ [raster.onLoad] 未找到占位符 ${placeholderId}，当前占位符数量: ${predictedPlaceholdersRef.current.size}`
                );
                logger.upload(
                  `⚠️ 未找到占位符 ${placeholderId}，将使用智能位置计算`
                );
              }
            }
          }
          ...
          if (targetBounds) {
            const sourceType =
              boundsSource === "selected" ? "选中图片边界" : "占位框";
            ...
            if (placeholderId) {
              logger.upload(
                `🗑️ [handleQuickImageUploaded] 准备移除占位符: ${placeholderId}`
              );
              const placeholderBeforeRemove =
                findImagePlaceholder(placeholderId);
              if (placeholderBeforeRemove) {
                logger.upload(
                  `✅ [handleQuickImageUploaded] 找到占位符，准备移除: ${placeholderId}`
                );
                removePredictedPlaceholder(placeholderId);
                logger.upload(
                  `✅ [handleQuickImageUploaded] 已移除占位符: ${placeholderId}`
                );
              } else {
                logger.upload(
                  `⚠️ [handleQuickImageUploaded] 未找到占位符，无法移除: ${placeholderId}`
                );
              }
            } else if (placeholder) {
              placeholder.remove();
            ...
            if (placeholderId && operationType && !finalPosition) {
              logger.upload(
                `⚠️ 占位符 ${placeholderId} 未找到，使用智能位置计算`
              );
              try {
                const calculated = calculateSmartPosition(
                  operationType,
                ...
                const heightForPosition =
                  expectedHeight || originalHeight || 512;
                const adjustedPoint = findNonOverlappingPosition(
                  desiredPoint,
                  widthForPosition,
                  heightForPosition,
                ...
                logger.upload(
                  `📍 使用智能位置计算: (${adjustedPoint.x.toFixed(
                    1
                  )}, ${adjustedPoint.y.toFixed(1)})`
                );
              } catch (error) {
          ...
          handles.forEach(({ direction, position }) => {
            const handle = new paper.Path.Rectangle({
              point: [
                position[0] - handleSize / 2,
                position[1] - handleSize / 2,
              ],
          ...
          window.dispatchEvent(
            new CustomEvent("quickImageAdded", {
          ...
          if (
            ...
            logger.upload(
              `🔗 [自动打组] 收集图片 ${groupData.imageIds.length}/${groupData.total}, groupId: ${parallelGroupId}, imageId: ${imageId}`
            );

            // 检查是否所有图片都已加载完成
            if (groupData.imageIds.length >= groupData.total) {
              logger.upload(
                `✅ [自动打组] 所有 ${groupData.total} 张图片已加载完成，触发自动打组`
              );
              ...
              setTimeout(() => {
                try {
                  ...
                  if (result.block) {
                    logger.upload(
                      `✅ [自动打组] 成功创建图片组，包含 ${imageIdsToGroup.length} 张图片`
                    );
                    paper.view?.update();
                    // 提交历史记录
                    try {
                      historyService
                        .commit("auto-group-images")
                        .catch(() => {});
                    } catch {}
                  } else {
                    logger.upload(
                      `⚠️ [自动打组] 创建图片组失败: ${result.reason}`
                    );
                  }
                } catch (err) {
          ...
          const positionInfo =
            boundsSource === "selected"
              ...
              : "坐标原点";
          logger.upload(
            `✅ 快速上传成功：图片已添加到${positionInfo} - ${
              fileName || "uploaded-image"
            }`
          );
          try {
            historyService.commit("add-image").catch(() => {});
          } catch {}
