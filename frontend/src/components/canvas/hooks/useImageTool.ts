...
import {
  syncImageGroupBlocksForImageIds,
  findImagePaperItem,
...
export const useImageTool = ({
  ...
  const handleImageUploaded = useCallback(
    (
      ...
      raster.onLoad = () => {
        ...
        if (!suppressAutoSave) {
          try {
            paperSaveService.triggerAutoSave("image-loaded");
          } catch {}
        }
      ...
      // 优先使用本地inline数据（data URL / blob / 纯base64），仅在没有本地数据时使用远程OSS URL（并通过proxy）
      const tryPrefixedDataUrl = (v?: string | null): string | null => {
        if (!v || typeof v !== "string") return null;
        const t = v.trim();
        if (t.startsWith("data:image/") || t.startsWith("blob:")) return t;
        // 纯base64字符串检测（长度与字符集）
        const base64Regex = /^[A-Za-z0-9+/]*={0,2}$/;
        if (t.length > 10 && base64Regex.test(t))
          return `data:image/png;base64,${t}`;
        return null;
      };

      let rasterSource: string = "";
      // 优先 localDataUrl，再 src，再 url（remote）
      rasterSource =
        tryPrefixedDataUrl(asset.localDataUrl) ??
        tryPrefixedDataUrl(asset.src) ??
        "";

      if (!rasterSource) {
        const remote = asset.url || asset.src || "";
        rasterSource = remote ? proxifyRemoteAssetUrl(remote) : remote;
      }
  ...
  const addImageSelectionElements = useCallback(
    (raster: paper.Raster, bounds: paper.Rectangle, imageId: string) => {
      ...
      try {
        ...
        parentGroup.addChild(selectionArea);
      }
    ...
    (imageId: string): string | null => {
      ...
      try {
        ...
        const raster = isGroup(imageGroup)
          ? (imageGroup.children.find((child) =>
              isRaster(child)
            ) as paper.Raster)
          : isRaster(imageGroup)
  ...
  const applyBoundsToPaperImage = useCallback(
    (
      imageId: string,
  ...
  const applyBoundsFromSnapshot = useCallback(
    (snapshots: ImageAssetSnapshot[]) => {
      ...
      try {
        syncImageGroupBlocksForImageIds(changedIds);
      } catch {}
      try {
        paper.view.update();
      } catch {}
  ...
  const applyPaperMoveToImage = useCallback(
    (imageId: string, newPosition: { x: number; y: number }) => {
    ...
    (
      ...
      if (!skipPaperUpdate) {
        validMoves.forEach(({ id, position }) => {
          try {
            applyPaperMoveToImage(id, position);
          } catch {}
        });
        try {
          syncImageGroupBlocksForImageIds(validMoves.map((m) => m.id));
        } catch {}
        try {
          paper.view.update();
        } catch {}
  ...
  const handleImageMove = useCallback(
    (
      ...
      handleImagesMove(
        [{ id: imageId, position: newPosition }],
        skipPaperUpdate
      );
    },
    [handleImagesMove]
  ...
  const setImagesVisibility = useCallback(
    (imageIds: string[], visible: boolean) => {
      ...
      try {
        const items = paper.project?.getItems({
          match: (item: any) => {
            ...
            const isImageGroup =
              data.type === "image" && typeof data.imageId === "string";
            const isRasterWithId =
              isRasterItem(item) && typeof data.imageId === "string";
  ...
  const handleImageResize = useCallback(
    (
      ...
      if (isGroup(imageGroup)) {
        ...
        try {
          syncImageGroupBlocksForImageIds([imageId]);
        } catch {}
        try {
          paper.view.update();
        } catch {}
  ...
  const hydrateFromSnapshot = useCallback(
    (snapshots: ImageAssetSnapshot[]) => {
      ...
      // 辅助函数：确保URL是有效的图片源
      const ensureValidImageSource = (url: string): string => {
        // 如果已经是有效的URL，直接返回
        if (
        ...
        // 检查是否是纯base64字符串（不含特殊字符如/+:等）
        const base64Regex = /^[A-Za-z0-9+/]*={0,2}$/;
        const trimmed = url.trim();
        // 长度合理且符合base64格式
        if (trimmed.length > 10 && base64Regex.test(trimmed)) {
      ...
      snapshots.forEach((snap) => {
        const rawUrl = snap?.url || snap?.localDataUrl;
        if (!snap || !rawUrl || !snap.bounds) return;

        // 确保URL是有效的图片源
        const resolvedUrl = ensureValidImageSource(rawUrl);
        if (snap.layerId) {
          try {
        ...
        if (placeholder) {
          ...
          // 确保localDataUrl也是有效的图片源
          const localDataUrl = snap.localDataUrl
            ? ensureValidImageSource(snap.localDataUrl)
            : resolvedUrl;
  ...
  const createImageFromSnapshot = useCallback(
    (
      ...
      const imageId =
        options?.idOverride ||
        `image_${Date.now()}_${Math.random().toString(36).substring(2, 11)}`;
