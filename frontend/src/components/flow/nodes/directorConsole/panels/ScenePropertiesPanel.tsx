...
import { HexColorField, Section, SliderField, Vec3Row } from './Field'
...
export function ScenePropertiesPanel({ scene, panoramaConnected, onPatch }: Props) {
  ...
  return (
    <div style={{ paddingBottom: 16 }}>
      ...
      <Section title="场景缩放">
        <SliderField value={(scene.sceneScale ?? 3) * 100} min={10} max={500} step={1} suffix="%" onChange={(value) => onPatch({ sceneScale: value / 100 })} />
      </Section>
      ...
      <Section title="天空颜色">
        <HexColorField label="天空颜色" value={scene.skyColor ?? '#060608'} onChange={(skyColor) => onPatch({ skyColor })} />
      </Section>
      <Section title="全景球">
      ...
      <Section title="透明度"><SliderField value={scene.groundOpacity ?? 0.4} min={0} max={1} step={0.01} displayDigits={2} onChange={(value) => onPatch({ groundOpacity: value })} /></Section>
      <Section title="高度"><SliderField value={scene.groundHeight ?? 0} min={-10} max={10} step={0.1} displayDigits={1} onChange={(value) => onPatch({ groundHeight: value })} /></Section>
    </div>
  )
}
