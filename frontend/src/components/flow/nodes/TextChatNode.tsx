...
4. Return only the converted result, without explanation or extra content.`
  ...
  React.useEffect(() => {
    ...
    if (data.textChatSkillId !== currentTextChatSkillId) {
      patch.textChatSkillId = currentTextChatSkillId;
    }
  ...
  const runChat = React.useCallback(async () => {
    ...
    const customPrompt = manualInput.trim();
    const inputPayload = latestIncomingTexts
      .map((text, index) => `#${index + 1}\n${text}`)
      .join('\n\n')
      .trim();
    if (!inputPayload.length && !(isCustomSkill && customPrompt.length)) {
      window.dispatchEvent(new CustomEvent('flow:updateNodeData', {
        detail: { id, patch: { status: 'failed', error: lt('请输入或连接至少一个提示文本', 'Please enter or connect at least one prompt text') } }
      }));
      return;
    }
    const skillPrompt = isCustomSkill ? customPrompt : currentTextChatSkill.prompt.trim();
  ...
  }, [bananaImageRoute, currentTextChatSkill.prompt, effectiveProvider, id, isCustomSkill, lt, manualInput, readIncomingTexts, textModel, updateIncomingTexts]);
