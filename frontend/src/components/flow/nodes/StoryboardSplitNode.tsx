      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          <button
            ...
            disabled={segments.length === 0 || outputCount === 0}
            style={{
              fontSize: 12,
              padding: '4px 10px',
              background: segments.length > 0 && outputCount > 0 ? '#059669' : (isFlowDark ? '#3f3f46' : '#9ca3af'),
              color: '#fff',
              borderRadius: 6,
              border: 'none',
              cursor: segments.length > 0 && outputCount > 0 ? 'pointer' : 'not-allowed',
              opacity: segments.length > 0 && outputCount > 0 ? 1 : 0.6,
            }}
