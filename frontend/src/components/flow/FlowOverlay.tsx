...
function FlowInner() {
  ...
  React.useEffect(() => {
    ...
    // 🔥 修复：统一处理所有节点，确保 running 状态被重置为 idle
    const sanitizedNodes = nextNodes.map((node) => {
      const nodeData = { ...(node.data || {}) };
      // 强制重置 running 状态为 idle，避免页面加载时进度条重新跑
      if (nodeData.status === "running") {
        nodeData.status = "idle";
      }
      return { ...node, data: nodeData } as RFNode;
    });

    setNodes((prev) => {
      // 如果是项目切换或首次加载，直接使用处理后的节点
      ...
      return sanitizedNodes.map((node) => {
        const prevNode = prevMap.get(node.id);
        if (!prevNode) {
          return node;
        }
        // 合并数据，但确保 status 不会被覆盖为 running
        const mergedData = { ...(prevNode.data || {}), ...(node.data || {}) };
        // 🔥 再次确保：如果合并后的状态是 running，强制重置为 idle
        if (mergedData.status === "running") {
  ...
  const runNode = React.useCallback(
    async (nodeId: string) => {
      ...
      const persistImagesToHistory = async (
        images: string[],
        baseName: string
      ) => {
      ...
      if (node.type === "generate4") {
        ...
        void persistImagesToHistory(
          produced.filter(Boolean),
          `flow-${nodeId}-gen4`
        );
        if (hasAny) {
          markTaskSuccess({
      ...
      try {
        ...
        markTaskSuccess({
          mode: "generate-single",
          imageCount: imgBase64 ? 1 : 0,
  ...
  React.useEffect(() => {
    ...
    setNodes((ns) =>
      ns.map((n) => {
        ...
        // 🔥 修复：页面加载时，即使有 running 状态的任务，也不应该将节点状态设置为 running
        // 因为任务可能已经完成或失败，只是任务存储中还有残留的 running 状态
        // 只有在任务真正还在运行时（通过其他机制确认），才应该设置为 running
        // 这里我们跳过 running 状态的任务，只处理已完成的任务
        const pendingTask = relatedTasks.find((t) => t.status === "running");
        // 注释掉自动设置 running 状态的逻辑，避免页面加载时进度条重新跑
        // if (pendingTask) {
        //   return {
        //     ...n,
    ...
    (id: string) => {
      ...
      if (node.type === "generate4") {
        ...
        imgs.forEach((img, idx) => {
          ...
          // 🔥 修复：检查 img 是否已经是完整的 data URL，避免重复前缀
          let dataUrl: string;
  ...
  return (
    <div
      ...
      className={`tanva-flow-overlay absolute inset-0 ${
        isFlowOnlyPointer ? "pointer-mode" : ""
      } ${drawMode === "global-pointer" ? "global-pointer-mode" : ""}`}
