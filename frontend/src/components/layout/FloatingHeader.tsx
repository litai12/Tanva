...
const FloatingHeader: React.FC = () => {
  ...
  const refreshBananaRouteSuccessRates = useCallback(async () => {
    try {
      ...
      console.warn("[FloatingHeader] Failed to load banana route stats:", error);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    refreshBananaRouteSuccessRates();
              ...
              >
                ...
                <div className='space-y-1'>
                  {bananaRouteOptions.map((option) => {
                    ...
                    const routeStats = bananaRouteSuccessRates?.routes?.[option.value] ?? null;
