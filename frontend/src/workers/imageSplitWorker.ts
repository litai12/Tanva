...
self.addEventListener("message", (event: MessageEvent<SplitImageRequest>) => {
  const data = event.data;
  if (!data || data.type !== "SPLIT_IMAGE") return;
  ...
  const run = async (): Promise<SplitImageResponse> => {
    try {
      ...
      const tooManyPieces =
        pieces.length > Math.min(MAX_OUTPUT_COUNT, Math.max(safeCount, DEFAULT_OUTPUT_COUNT)) * 2;
