...
- 非小T的 AI 文本能力统一经 new-api `POST /v1/chat/completions` 调用 GPT：普通文字对话、Flow Text Chat、提示词优化、工具选择与 PDF 文本分析使用 `gpt-5.4`；HTML PPT、Paper.js、图像转矢量和普通 Agent 规划/研究使用 `gpt-5.6-luna`。Image Chat/图片理解恢复 Gemini 多模态三档：Fast=`gemini-2.5-flash`、Pro=`gemini-3.5-flash`、Ultra=`gemini-3.1-pro`，统一由 `/api/ai/analyze-image` 执行。Flow 视频分析使用节点内独立模型选择，不继承全局图片模型：默认 `doubao-seed-2-0-lite-260428`，并提供豆包 Mini/Lite/Pro 与 Gemini 2.5 Flash/3.5 Flash/3.1 Pro；豆包经 new-api `/v1/responses` 的远程 `input_video` 分析，Gemini 保留完整小视频与抽帧兜底。视频分析结果生成标准 Prompt 的分镜表变体，表格/原文双向编辑，允许增删/复制行、新增/重命名/删除动态列和切换镜头/时序列作用域；内部镜头分组不得依赖用户可删除的“镜号”列。小T三档大脑仍为 Fast GPT-5.4、Pro GPT-5.5、Ultra GPT-5.6 Luna，默认 Fast，但图片分析必须调用宿主 `analyze_image` 工具复用 Image Chat，不由小T大脑直接猜图。
- Tanva 后端只通过 `NEW_API_BASE_URL` 与 `NEW_API_KEY` 访问 new-api；tc-api 的 base URL 与 `tc_sk` 只保存在 new-api 渠道配置中。部署使用的 new-api 必须为 `default` 分组启用普通 `gpt-5.4`、`gpt-5.6-luna` 以及视频分析所需的 `doubao-seed-2-0-mini-260428`、`doubao-seed-2-0-lite-260428`、`doubao-seed-2-0-pro-260215` abilities；缺少网关 ability 时应修复渠道，不得让后端直连上游。积分记录与客户端模型外显统一标记为 new-api + 实际模型。可运行 `cd backend && npm run verify:new-api-text-routing` 与 `npm run verify:new-api-video-analysis` 做无付费 mock 路由验证。
