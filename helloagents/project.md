...
- 小T 上游 SSE 结束只表示传输结束。正文、完整 `flow_patch`、`host_tool` 与 `host_ui` 至少要存在一种，后端才允许结算成功回合并发送 `final/done`；tool-call JSON 截断或四类交付全部为空时 run 显式失败。前端还必须通过本轮交付证据校验，禁止用“任务已完成”替换空的思考占位。
