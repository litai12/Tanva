...
- Flow Prompt 节点的 `@` 图片引用保存为结构化 `data.mentions`：新建引用从项目库与个人库选择并只保存远程 URL/路径；历史当前工作流引用仍可按节点/句柄兼容解析。运行时可合并到生图参考图，但不得把 inline 图片写入设计 JSON。Prompt 不再从下游节点反查图片并展示为可插入候选；已选引用在 Prompt 输入区以图片 chip 区分展示，删除时应按整个 token 同步清理 mention。
