...
- 小T请求的内容安全与敏感话题判断由小T facade 渠道自身负责；可选大脑仅为 `xiaot-agent-gpt-5-6-luna`、`xiaot-agent-gpt-5-6-terra` 和 `xiaot-agent-deepseek-v4-flash`，用户外显分别为 `小T-5.6 Luna`、`小T-5.6 Terra`、`小T-DeepSeek V4 Flash`，默认与旧/非法偏好统一回退 Luna。Tanva 不得把完整画布和完整能力说明无条件注入每轮：首轮只发送节点计数/类型摘要、当前选中节点和由请求明确命中的最多 8 个节点，能力清单只保留执行结构；内联/超长内容必须剔除。纯“你好/Hi”等问候是唯一允许的本地快路径，即时回复、不调用上游、不扣小T对话积分；其余内容安全与生成判断仍交给 facade，不新增本地安全 Guard。
...
- 小T 上游 SSE 结束只表示传输结束。后端必须同时收到与实际交付通道一致的终帧 `finish_reason`（纯正文=`stop`，存在宿主调用=`tool_calls`）和 `[DONE]`，且正文、完整 `flow_patch`、`host_tool` 与 `host_ui` 至少存在一种，才允许结算成功回合并发送 `final/done`；上游 `error`（包括 `xiaot_turn_suspended`）、缺终帧、tool-call JSON 截断、finish reason 冲突或四类交付全部为空时均显式失败。前端事件流也必须真实收到 `done`，并把上游 `error` 作为否决完成态的结构化证据；部分正文不能把失败回合标记为“已完成”。Tanva→TapCanvas 的稳定会话键使用 `xiaot-v2:` 协议前缀，旧 v1 上游历史保留但不再注入新回合，避免历史工具契约失败继续污染模型。
