...
- 非小T的 AI 文本能力统一经 new-api `POST /v1/chat/completions` 调用 GPT：普通文字对话、Flow Text Chat、提示词优化、工具选择与 PDF 文本分析使用内部模型 `gpt-5.4`；HTML PPT、Paper.js、图像转矢量和普通 Agent 规划/研究使用内部模型 `gpt-5.6-luna`。所有面向用户的对应型号标签仍统一使用“小T”品牌前缀（例如 `小T-5.4`、`小T-5.6 Luna`），不得外显 `GPT-*`，内部请求 ID 不变。Image Chat/图片理解恢复 Gemini 多模态三档：Fast=`gemini-2.5-flash`、Pro=`gemini-3.5-flash`、Ultra=`gemini-3.1-pro`，统一由 `/api/ai/analyze-image` 执行。Flow 视频分析使用节点内独立模型选择，不继承全局图片模型：默认 `doubao-seed-2-0-lite-260428`，并提供豆包 Mini/Lite/Pro 与 Gemini 2.5 Flash/3.5 Flash/3.1 Pro；豆包模型请求经 new-api `/v1/responses` 的远程 `input_video` 分析，Gemini 保留完整小视频与抽帧兜底。Lite 分析计费锚定标准付费 Seedance 2.0 480P 单价的 `1/3`：当前为 `1.25 元/秒 ÷ 3 = 125/3 积分/秒`，按后端安全下载并用 `ffprobe` 识别的完整视频时长计算总价后向上取整；浏览器媒体时长只用于运行前预估。Mini/Pro 继续按 Responses 的真实输入/缓存/音频/输出 token，套用火山官方 `0–32K`、`32–128K`、`128–256K` 分档价，再以 `官方人民币成本 × 1.5 × 100` 计算整数积分。禁止任何豆包模型回退到 `60/90/120` 固定扣费。公开 API 可返回最终积分及 Lite 计费时长，但不得返回官方成本、倍率、分档单价或 `pricingSnapshot`；完整快照仅保存在服务端供管理员审计。视频分析结果生成标准 Prompt 的分镜表变体，表格/原文双向编辑，允许增删/复制行、新增/重命名/删除动态列和切换镜头/时序列作用域；内部镜头分组不得依赖用户可删除的“镜号”列。表格态必须支持单元格内 `@` 工作流/项目库/个人库资产、已引用资产定位、常驻横向浏览控件与 `Shift + 滚轮` 横移，并支持 `.xlsx` 导入/导出；工作簿使用“分镜表/镜头总览/列设置”三个 Sheet 保留动态列和作用域。小T大脑默认 Fast，另可选 Pro、Ultra 和 DeepSeek V4 Flash；图片分析必须调用宿主 `analyze_image` 工具复用 Image Chat，不由小T大脑直接猜图。
...
- Flow 视频普通/尊享通道使用不同的 new-api 入口：普通通道经 `/v1/videos` 使用后端 `NEW_API_KEY`；`vendorKey/platformKey=tencent_vod|tengxun` 的尊享通道使用 Tencent VOD。真实令牌不得下发前端；type=67 Tencent VOD task channel 通过 Tanva 内部 create/poll endpoint 执行腾讯签名请求，模型必须具备对应 group ability 后才能送入 `/v1/videos`，未注册 ability 时不得静默回退或绕过 new-api 计价。
...
- Hailuo H3 是异步计费：new-api 返回任务后必须按 `X-NewApi-Consumed-Credits` 创建 `PENDING` 用量记录，返回并持久化 `apiUsageId/taskId`；只有任务成功才回写 `video-task-success`，失败走 `video-task-refund`，视频 pending 超时自动退款名单必须包含 `hailuo-video`。团队项目先冻结团队积分，终态成功才确认扣除，失败只释放预留。
...
- Seedance 2.5 在同一 `seedance-2.0` 托管模型下以子型号 `seedanceModel=seedance-2.5` 接入，上游必须精确发送 `doubao-seedance-2-5`。当前前端选择器只保留 `Seedance 2.5（即将推出）` 产品预告，并必须禁用该选项、阻止用户新选；后端兼容链路继续保留供历史数据与后续开放使用。它仅走 Ark 官方普通通道 `seedance_api/default`，前端不展示不适用的 Tencent VOD 尊享选项，后端还会清除历史节点残留的 VIP 路由，禁止静默执行成其他版本。2.5 复用 Seedance 2.x 的模式、一次性 Ark 素材与总处理时长计费链路，仅允许 `480P/720P`；前端切换型号时清理非法分辨率，后端提交前再次拒绝 `1080P/4K`。对应档位的每秒价格固定为当前标准 2.0 的 `1.5x`：480P=`1.875 元/秒`、720P=`2.25 元/秒`，不参与仅面向 2.0/Fast/Mini 的 `SEEDANCE20_FREE` 活动。new-api 生产注册与 ability 克隆使用幂等补丁 `new-api/patches/2026-07-31/001-add-doubao-seedance-2-5.sql`。
