...
- Flow/Prompt Mentions: provider-video nodes (`Seedance/Kling/Vidu` family) now merge active Prompt `@` image mentions into runtime reference images ahead of connected image handles; Seedance validation counts those mentions as image references, video prompts get explicit `图1`/mention-to-reference mapping, and old previews are cleared when a new run starts.
