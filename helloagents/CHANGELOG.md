...
- 2026-09-25：修复 101 Seedance 参考素材上传到 `default`、生成 API Key 属于 `beq` 导致提交时报素材不存在。后端素材项目默认改为 `beq`；审核组、任务组和真人认证组保存项目归属并安全清理旧 `default` 记录，旧真人认证需重新进行。增加项目切换回归验证；已提交代码，待用户部署。

- 2026-09-21：修复小 Tai 回退提交导致的前端构建失败：将新增 import 移到原有 ts-nocheck 指令之后，恢复指令有效性；修复视频下载失败兜底引用 try 内局部变量的问题。

- 2026-09-21：小 Tai 对话临时退回旧版 Auto / 手动文字和图片执行链路，暂停 xiaotagent 与 Beta 入口；阻止缓存重新启用，后端默认关闭 canvasAgent。聊天视频提示使用画布节点。保留接入实现便于恢复；仅本地修改，未发布。

- 2026-09-20：修复图片重复执行与超时误退款：取消控制器/Provider 重发，阻断重复扣费句柄继续生成，Worker 原子抢占并保存计费身份，未知结果保留待核实；图片轮询等待真实终态，排队取消必须确认。新增并发/断网/退款/轮询回归，详见 helloagents/wiki/image-terminal-billing-20260920.md。

- 2026-09-19：修复节点对齐后叠压及工具栏被聊天框遮挡；对齐时自动补足间距，工具栏实时避让聊天浮层，对齐后在可用画布区域缩放居中。补充布局、视野与浏览器组件回归；本地修改，未发布。

- 2026-09-19：新增节点框选多选工具栏，支持六向对齐与打组；按实际尺寸计算选区，保留未选中节点和组内相对布局，兼容协作锁定、自动保存与撤销。新增布局回归测试；仅本地修改，未发布。

- 2026-09-14：修复单图节点准备阶段缺少即时反馈，以及停止后后台继续生成却允许再次提交的问题。运行入口立即显示进度、清除旧任务身份，准备异常显示失败；停止保留原任务轮询和防重锁直至真实终态。专业生图节点增加阶段文字。仅本地前端修改，未发布；详见 `wiki/image-generation-submission-feedback.md`。
...
- 2026-09-10：完成 GPT Image 模型切换的 new-api/Nest 配套审查。101 六个相关源码文件与本地一致，已有渠道及公开目录正常；网关三包测试、后端质量/目录/计价回归与构建通过。补充 Jichuan 请求与普通 token 选择测试、前端跨型号目录积分切换断言。本次运行变更仅需发布前端，不需数据库迁移或重建网关；提交部署由用户执行。

- 2026-09-10：curl 实测 101 Jichuan GPT Image 2.5 文生图和单图参考均 HTTP 200，分别约 36/38 秒，目视确认红杯改蓝杯。前端 GPT 节点补充 2 / 2.5 切换，同步路由身份、质量默认值与计费配置，保留输入/结果并遵循目录启停；4 项回归与生产构建通过。前端未发布，本地 4458 尚缺该渠道。详见 `wiki/gpt-image25-jichuan.md`。

- 2026-09-09：修复 Group 边框按初始高度计算导致图片节点溢出；优先真实测量尺寸并移除跳过下一轮更新的锁，保留成员排版。补充动态增高/缩小、负坐标、组外节点排除与未测量回退测试。仅本地修改，未发布。

- 2026-09-09：修复小T新增节点及回合完成自动整理全画布的问题；新节点在当前视窗避让旧节点与聊天浮层，通过提示词说明默认新建、仅在用户指定节点或框选时操作旧节点，保留旧图片；按用户要求不增加宿主保护，文本节点统一为 Prompt。仅本地修改，未发布。
...
- 后端固定 DeepSeek 专属门面，旧请求、配置和续跑参数不再启用 GPT；新增单模型路由、失败不补发/不扣费及历史偏好迁移回归。
- 前后端构建、模型路由/计费、偏好迁移与宿主恢复回归通过；相关独立前端模块 lint 通过，全量 frontend lint 未通过（2563 errors / 199 warnings）。未部署。

## 2026-09-07 小T断流后无主挂起修复包（未部署）

- 根据 101 的 Luna 连续两次 `provider_terminal_missing` 日志，提取 TapCanvas 上游最小源码补丁：断流重试耗尽转 `replan_required`，恢复时优先使用真实 physicalRunId。
- 增加旧线上产物本地复现、本地源码/产物回归与特定版本隔离构建；保留 Tanva 对真实无主挂起的失败校验。源码补丁、验证方式与发布范围见 `backend/patches/2026-09-07-xiaot-continuation/README.md`。用户自行部署，线上服务未切换或重启。

## 2026-09-07 聊天视频改由画布节点生成

- 移除聊天固定调用 Seedance 1.5 Pro 的独立提交、轮询和退款路径，聊天视频统一由小T创建画布节点并按节点规格生成；补齐参考图传递和真实结果验收。
- 后端对已停用的 1.5 Pro 在扣分和上游调用前拒绝请求。


## 2026-09-07 后台积分明细筛选与分页

- 用户细分积分流水增加 20/50/100 条分页、起止日期和模型关键词查询，取消固定仅加载最近 100 条的限制；服务端按完整账户流水筛选并返回准确总数。


- 2026-09-07：按业务要求统一积分消耗顺序为签到 → 管理员手动充值 → 系统邀请 → 会员 → 单独购买；识别 admin_add 赠送批次与历史 manual，四类个人预扣/补扣入口共用排序，增加跨来源优先级与退款回归。

- 2026-09-07：统一白名单新增独立“SD2 使用权限”，仅开放 Seedance 2.x；普通用户签到仍为基础 50 积分，未用部分次业务日清除，不附带会员或其他白名单权益。补齐数据库迁移、后台开关与权限/签到清理回归。
- 2026-09-07：本地修复免费积分消费优先级，覆盖预扣、精确扣费和终态补扣；旧邀请奖励在账户锁内按现有未分批余额迁移，消费和退款共用批次余额。移除旧奖励原额直接衰减，未审计旧奖励只消费不自动衰减，正常赠送规则保留。新增生产排序复现、混合扣费退款、余额守恒、迁移幂等和实际衰减服务回归；尚未部署。
- 2026-09-07：完成两名指定用户历史邀请奖励消费归属矫正，返还误衰减 250 / 350 分并恢复相应充值批次，当前这几笔奖励可衰减额归零。新增带预览指纹、事务锁、原始快照、幂等标记及证据校验的数据修复脚本与回归验证；本次未修改全站消费排序。
- 2026-09-07：修复视频网关网络失败被显示为笼统 500 的问题，统一返回明确 503 提示并沿用退款；重建本机 4458 网关及 PostgreSQL/Redis 卷，从补丁恢复配置和 Wan3.0 渠道。补丁 runner 对缺少凭据的小T渠道及其依赖保留待执行状态，避免阻塞后续模型初始化。
- 2026-09-07：补齐 Wan3.0 首帧/首尾帧、多图参考和参考视频生成/编辑的画布输入与端到端媒体转发，复用公共节点组件并按连接显示模式；后端校验媒体组合、远程地址和真实参考视频时长，网关支持仅媒体请求。新增连线容量、非法组合、时长、计费及网关媒体保真回归。
- 2026-09-07：修复 Wan3.0 提示词连线遗漏类型及容量校验的问题；界面复用 Seedance 的 GenericVideoNode，统一按钮、积分、预览和历史操作，时长统一 clipDuration 并兼容旧 duration。新增实际连线回调回归检查。
...
- 2026-09-03：任务日志改为长期保留在数据库，不再随后台 3 天清理任务删除；任务日志页新增按当前筛选条件一键导出全部记录的 CSV 下载，管理员可查看用户列，普通用户仅导出自己的任务。

- 2026-09-03：修复免费积分 lot 到期后仍显示 active、导致展示与扣费不一致的问题。余额汇总现在排除已过期 lot；每日到期清理不再因用户有历史充值而跳过；个人生图/其他积分扣费前会先收敛过期免费额度，确保有效免费积分按策略优先消耗，过期额度不再进入扣费或衰减。

- 2026-08-31：治理生产 Node 图片任务进程的原生内存长期滞留。保持 BullMQ 并发上限 1000，同时增加 64MB 结果硬上限和受限下载、去除 OSS 上传的重复 Buffer、收紧 Sharp cache/concurrency/pixel、按需加载 PDF/Skia，并让 PM2 优先使用 jemalloc；RSS 达 2GB 时仅在本进程无 active job 后安全回收，6GB PM2 阈值作为紧急保险。系统监控同步改为显示真实 PM2 阈值与 allocator，不再用硬编码 4GB 误报“会自动重启”。

- 2026-08-31：恢复并收紧免费积分生命周期。签到清理按执行时的当前有效月卡、年卡或 VIP 白名单动态判断：有资格时暂停；资格失效后所有已跨凌晨 3 点业务日边界的签到积分（包括会员期间累计与历史无 lot 记录）一次性清除，当日签到仍可使用到下一业务日。签到 gift 排除每日 50 衰减并以 `priority=-200` 优先消费。历史 VIP 签到误清积分由迁移自动补回，资格元数据只保留审计含义，不构成永久豁免。非有效 VIP 用户的邀请奖励、运营赠送和受邀注册免费额度等其他 gift 继续在每日 2 点默认衰减 50；充值本金和充值赠送统一归为不可衰减的 recharge。

- 2026-08-31：Tanva 后端撤销 Seedance 2.0/2.5 参考视频与参考音频的单条/总时长范围拦截，不再因 MP4 容器尾差（如 `15.01s`、`15.1s`）在本地拒绝，也不做 ffmpeg 裁切或重编码；原始远程素材引用直接进入计费和上游提交，最终支持范围及错误由 new-api/Seedance 上游裁决。素材数量、任务类型、输出时长和计费所需的真实参考视频时长探测保持不变。

- 2026-08-31：Flow 的历史 `omniFlashExtVideo` 节点保留类型和 `managedModelKey=omni-flash-ext` 以兼容旧画布，产品名更新为 Gemini Omni Flash；实际上游使用 ToAPIs 文档允许且当前渠道已配置的 `gemini_omni_flash` 别名（文档默认写法为 `gemini-omni-flash`）。节点仅保留 720P/1080P 和 4/6/10 秒，1080P 仅支持 16:9，参考视频输入会被拒绝。用户价按供应商成本 `x1.5` 计算并向上取整积分：720P 4/6/10 秒=`158/158/210`，1080P=`210/210/263`。new-api 增加同矩阵的提交前计费因子与幂等生产补丁。

- 2026-08-30：Flow 顶栏“一键整理”改为选区优先：存在选中节点时只按类别整理选中的原子节点/节点组，未选中节点保持原位；没有选区时继续整理全画布。小T新增节点触发的自动整理仍固定使用全画布范围，不受用户遗留选区影响；按钮提示与成功反馈会明确当前语义。

- 2026-08-26：修复旧价格版本月付会员无法换购当前套餐。旧月付用户现在可全额换购同档或更高档的 `2026-08-v2` 套餐；付款后旧套餐立即结束、旧会员额度清零，新套餐从付款时刻重新起算完整周期，不叠加剩余天数。独立充值与永久赠送积分保持不变；订单会绑定下单时的旧订阅 ID，回调时套餐若已变化则暂停自动覆盖，避免误清新套餐额度。
...
- 2026-08-21：修复桌面顶栏画布开关在窄线程区被压成两行，以及小T执行中手动隐藏画布后又被后续 `flow_patch` 自动顶开。开关固定为单行“画布 / 隐藏”，收紧顶栏项目选择宽度；新增本轮自动唤起抑制，用户手动隐藏优先，直到主动打开、切换会话或发送下一条指令才解除。

- 2026-08-21：完成一轮本机 Codex 桌面运行时逐项对齐。项目行只展开/收起，常驻 `+` 可直接创建并绑定项目会话；Work、Chat、项目目录均可折叠。新增项目/会话置顶、会话重命名/移动/归档/恢复/删除、菜单失焦互斥、搜索上下选择、`⌘B` 侧栏显隐及宽度持久化；文件工作台补齐 PPT 逐页缩略图、大页预览和 HTML/PPTX 导出。完整矩阵记录于 `frontend/docs/desktop/04-Codex交互对齐清单.md`。

- 2026-08-21：补齐 Codex 项目直接发起任务入口。每个项目行常驻显示 `+`，点击后原子完成“创建全局桌面会话 → 绑定项目 → 展开 Work/项目 → 打开项目上下文 → 切换新任务”；`WORK` 标题右侧 `+` 继续只负责创建项目，避免入口职责混淆。浏览器与打包应用验收覆盖从项目创建任务、移动回 Chat 后继续留存及测试任务清理。

- 2026-08-21：桌面侧栏交互进一步按 Codex 状态机收口。Work 一级目录与所有项目二级目录均可折叠并持久化；点击项目行或箭头可展开/收起，任务拖入时自动展开 Work 和目标项目。修复拖入项目后任务消失：Electron 任务会话改为全局本地任务存储，项目画布加载不再通过 `Project.content.aiChatSessions` 重置桌面任务。项目/任务/扩展/账号弹层补齐点击外部、窗口失焦与 Escape 关闭，同层菜单互斥；重命名失焦保存，空的新建项目输入失焦取消。浏览器验收新增真实拖入后留存、Work/项目两级折叠和弹层失焦关闭断言。

- 2026-08-21：补齐 Codex 式桌面项目管理。`WORK` 标题右侧新增唯一项目创建入口，项目行新增持久化展开/收起和唯一 `…` 管理菜单，支持重命名、二次确认删除；删除项目会将绑定任务迁回 Chat。桌面端移除“无项目自动创建”和“删除最后项目自动补建未命名项目”的网页遗留副作用，点击 Work 且尚无项目时改为复用同一显式创建框。新增项目上下文清理与折叠状态回归测试。

- 2026-08-21：桌面登录改为长期设备会话。Electron 使用系统 `safeStorage` 加密保存 access/refresh token，首次升级自动迁移并删除旧 localStorage 明文；应用挂载前恢复登录态，弱网先进入本地桌面再后台按 JWT 真实签发时间续期。后端刷新令牌由“只校验账号最新 token”改为匹配当前设备实际提交的 token，刷新和退出只轮换/撤销本会话，网页登录或其他设备登录不再把桌面误顶下线。

- 2026-08-21：修复桌面端快速展开/收起画布时的 Paper.js 生命周期竞态。画布销毁会取消所有迟到的缩放、布局与尺寸回调，并只清理本次挂载对应的 Project/View，避免旧回调向已释放视图写入 `matrix` 导致错乱或白屏；浏览器验收新增文件工作台截图并重新通过全链路切换检查。

- 2026-08-21：桌面任务增加 `Work / Chat` 归属。新任务默认 Chat；任务可拖入项目成为 Work，也可拖回 Chat；只有 Work 展示项目选择与画布入口。左下角账号区新增个人/团队、余额和团队切换，切换后同步项目与计费上下文。

- 2026-08-21：新增 `tanva.artifacts` 文件工作台。PPT 生成后进入独立文件交付面并可回到画布编辑；新增 `create_spreadsheet`，支持多工作表预览和有效 XLSX 导出，不要求画布承载 Excel。

- 2026-08-21：真实复现小T facade 返回 `402 team_insufficient_credits`。这是 new-api 模型渠道额度，与左下角 Tanva 个人/团队积分不是同一本账。前端与后端现在都在 Luna、Terra、DeepSeek 可用线路间去重回退，任一线路成功即正常交付并只扣 Tanva 固定对话积分；全部渠道不可用时明确提示“与你的 Tanva 积分余额无关”，不再误导用户充值或二次包装成“上游回合以错误终态结束”。

- 2026-08-21：收口 Electron 画布集成态。当前任务顶栏新增唯一“画布 / 收起画布”开关；精确“打开/收起画布”对话命令改为本地宿主动作，不请求小T上游。Canvas 新增 `embeddedDesktop`，隐藏网页项目头、账号/积分导航、Focus 全局模式和第二套小T，并把工具栏、图层、素材、评论与缩放约束在右侧插件容器，避免覆盖任务边栏。前后端同时识别 TapCanvas 的 `xiaot_turn_suspended + host_execution_required/root_physical_execution_budget_exhausted` 为画布命令交接而非错误，继续等待真实节点/资产证据；错误展示改为解析完整上游信封，不再把结构化错误截成 `{`。所有内置 public 图片/视频/模型引用统一经桌面相对资源解析，修复 Logo 与小T头像重复裂图；浏览器验收新增手动开关、本地命令和嵌入边界检查。

- 2026-08-21：修复 Tanva Electron 安装包启动后纯白屏。根因是 Vite 生产 HTML 使用 `/assets/...` 根绝对路径，而打包应用通过 `file://` 加载，导致 JS/CSS 指向磁盘根目录；桌面构建现使用独立 `VITE_DESKTOP_BUILD=1` 相对资源基址，Electron 路由切换为 Hash Router，并停止在桌面环境注册网站 Service Worker。新增打包前资源路径断言、Renderer 加载/控制台诊断和“React 根节点有真实内容 + 5 个连接器 IPC”的加强冒烟，避免空 `#root` 再次误报成功；macOS arm64 App/DMG/ZIP 已重建并通过打包应用冒烟。

- 2026-08-21：完成 Tanva 桌面版第一轮产品与交互设计。v2 原型加入“一个功能只有一个唯一入口”的硬约束；v3 恢复 Tanva 几何蓝色品牌、纯白/Premium Black 画布、轻量液态玻璃、小T彩雾任务光环与既有 React Flow 节点；v4 进一步把整体框架纠正为 Codex 式“任务边栏 + 当前小T任务线程 + 可开合侧面工具面”，Tanva 画布对应 Codex 的 Browser panel，由小T在当前任务中调用，支持侧栏展开、最大化、恢复与关闭，不再设置“小T / 项目 / 任务 / 扩展”图标轨或“对话 / 协作 / 画布”三态切换器。JZXZ 继续作为首批文档、知识库/RAG、检索、MCP 与本地任务能力包；原型统一通过 TapCanvasPro 自有 `gpt-image-2` 链路生成。

- 2026-08-20：小T新增浏览器内演示文稿宿主工具 `create_presentation` / `edit_presentation`。用户要求 PPT、幻灯片、提案或建筑设计汇报时，小T会创建/定位 `htmlPpt` 节点，连接当前选中素材和消息附件，等待节点真实生成回执后在聊天中返回演示文稿成品卡；卡片支持打开画布、下载 HTML 和高保真 PPTX。默认新增 Tanva Studio 黑白设计系统，建筑模式明确禁止虚构面积、法规、结构、造价等工程事实，并要求优先展示图纸证据、体量、功能流线、气候与材料逻辑。整套改写按最多 3 页一批生成并校验精确页数，全部批次成功后原子提交；渠道不可用/503 时自动尝试兼容文本模型。PPTX 导出按需加载 MIT 开源 `PptxGenJS` + `html-to-image`，将固定设计画布逐页栅格化封装，优先保证浏览器预览与 PowerPoint 视觉一致。

- 2026-08-19：修复“参考图1/图2生成展示图”被“参考”一词误判为联网案例研究、继而发送已废弃 `web_search_preview` 并报 400。带图片请求现在先进入图片编辑/融合链路；前端不再用关键词自动打开联网，显式联网文本请求硬切 new-api `/v1/responses` + `web_search`，`thinking_level` 转为 `reasoning.effort`。删除联网失败后偷偷移除搜索的重试，失败会明确返回；新增礼盒参考图分流与 new-api payload 的无付费回归验证。

- 2026-08-19：修复 Prompt Optimizer 的 Terra 请求固定在 25 秒被前端误杀。三模型优化请求现允许单次等待 120 秒，并把该时限传入真实文本请求的 `AbortController` 与认证请求层；删除旧的 Promise 外层假超时及自动重试，避免界面已报错但底层请求仍运行、随后重试造成重复上游调用或重复计费。

- 2026-08-19：修复“小T只问你好也很慢”和每轮无条件上传完整画布的问题。纯问候由 Tanva 本地即时回复，不再启动上游代理、消耗 token 或扣对话积分；其他回合初始上下文改为画布节点计数/类型摘要，只附带当前选中节点及由请求明确命中的少量节点，单次最多 8 个，`data:`/`blob:`/超长字段会被剔除。40KB 完整能力清单压缩为节点执行结构，去掉长篇用途说明；后端新增受限画布查询器，限定 `summary/selected/ids/neighbors/search` 五种局部查询和 12 节点/24 连线硬上限。提示词优化 facade 每次使用独立临时会话，固定 `chat` 模式并禁用代理工具，避免继承小T画布、记忆或历史工具回合。

- 2026-08-19：小T大脑收口为 GPT-5.6 Luna、GPT-5.6 Terra、DeepSeek V4 Flash 三种 facade，默认 Luna，旧 5.4/5.5 偏好自动迁移。Flow Prompt Optimizer 与 AI Chat 提示词扩写面板同步开放三模型选择，Flow 节点恢复紧凑胶囊点击切换，节点标签、报价和运行请求读取同一产品模型；后端按 `prompt_optimize` 白名单强制走 new-api，并在最后一跳转换为对应 `xiaot-agent-*` 门面，复用小T已配置的大脑渠道，避免裸 Luna/Terra/DeepSeek 没有 ability 的 503。新增前后端无付费回归与覆盖 `default/auto/vip/svip` 的小T facade 补丁 `001-xiaot-luna-terra-deepseek-models.sql`。

- 2026-08-18：补齐托管模型单独下线到 Flow 型号菜单的闭环。Seedance 1.5 Pro（`seedance-1.5`）与 2.x（`seedance-2.0`）不再共享 `supportedModels` 归属；关闭 1.5 只移除其独立节点并把统一 Seedance 菜单中的 1.5 标为“已下线”，不影响 2.0/2.5/Fast。修复禁用键被清空后节点误通过可见性过滤的问题；历史节点不再自动切换型号，使用最新运行时配置展示状态并禁用 Run。
- 2026-08-17：新增节点模型“用户消费运营”配置。厂商 `pricing.v2.consumerPolicies` 可选择整条节点线路，或按型号、分辨率等规格配置限时折扣/单独禁用；折扣只改变 Tanva 最终扣减积分，不修改刊例价格、上游成本和请求参数，开始时间包含、结束时间不包含，到期自动恢复刊例积分。Seedance 2.5 1080P 于 `2026-08-14 14:00` 至 `2026-09-17 14:00`（Asia/Taipei）按刊例积分 `72%` 计费：5 秒刊例 `2813` 积分按精确金额折算后实扣 `2025`；4K 暂停开放，Flow 显示“暂未开放”且后端同步拦截。后台管理新增策略范围、优先级、折扣、禁用文案和时间窗编辑器。

- 2026-08-17：Seedance 2.5 补齐 1080P / 4K。Flow 选择器、后端提交守卫、管理端定价模板与 new-api catalog 统一开放 `480P/720P/1080P/4K`；新增高分辨率线性计费规则，继续按标准 Seedance 2.0 同规格当前商业单价 `×1.5`：1080P=`5.625 元/秒（562.5 积分/秒）`，4K=`11.25 元/秒（1125 积分/秒）`。任务总积分仍在完整时长价格计算后统一向上取整，2.5 仍不参与 `SEEDANCE20_FREE`。
...
- Payment UX: “自定义积分充值”仅对超级管理员（`role = admin`）展示；普通用户和普通管理员继续使用固定档位，后端同步拒绝非超级管理员提交非固定档位订单。

- Membership purchase UX: 当前套餐及非升级目标的购买按钮置灰；只有当前可售最高档年卡的生效会员购买独立积分充值包时享 8 折，最高档年卡的标题说明与权益列表均明确展示该权益，充值区对未开通用户展示“年费旗舰会员享受 8 折”、对已生效用户展示“当前已享年费旗舰会员 8 折”；后端统一判定资格、报价、校验和落单，月卡和其他年卡均按充值原价购买，套餐订阅价格不参与该充值折扣。
...
- Flow/Seedance 2.5：正式开放选择器并升级方舟上游模型至 `doubao-seedance-2-5-260628`；输出时长扩展为 4–30 秒，多模态参考能力按官方规格扩展为 30 张图片、10 条视频、10 条音频，视频/音频单条与合计时长均校验为 2–30 秒，2.5 可仅使用音频作为输入。端点悬浮提示按 2.5 显示 30/10/10 与时长上限。`seedance-2.0` 保留为内部路由键，但请求提交前会强制写入 2.5 的 Ark 快照 ID，避免旧数据库模板将实际模型降级为 2.0。新增 new-api 幂等迁移，把已有的旧 2.5 catalog/ability 升级到当前快照模型 ID；即使旧 2.5 ability 缺失，也会直接从已启用的 Ark 2.0 ability 克隆 `default` 路由。
- Flow/Seedance 2.5：取消 Tanva 侧的 5000 字符提示词拦截。前端最终 prompt、DTO 校验及后端 new-api 提交均允许完整透传；其他视频模型继续保留原有长度保护。
...
- Frontend/Hailuo H3：端点悬浮提示改为复用 Seedance 的动态端点布局，并从 new-api 模型目录展示图片、视频、音频输入上限；当前 H3 的图片/视频上限为 9/3，前端连线与运行校验继续按该规格拦截，已保存模型规格可在目录请求失败时回退使用。
- Frontend/Hailuo H3：修复物理图片连线与 Prompt `@` 图片引用重复计数的问题；同一张图只计一次，8 张图片不再因重复计数被误判为超过 9 张上限。
- Frontend/Referral Check-In：签到成功后直接使用接口返回结果更新状态并刷新余额，移除点击后的整块面板加载态、“签到中”文案和成功确认弹窗；失败信息改为面板内提示。
...
- Gateway: expanded the Ali task adapter and managed model catalog for four official Wan request shapes and four HappyHorse models, preserving `media[]`, `reference_video_urls`, `ratio`, and `shot_type`. Added canonical routing, video pricing, an idempotent PostgreSQL patch, Go adapter tests, and a no-cost backend routing verifier.
- Admin/Flow metadata: Wan and HappyHorse managed vendors now identify as `new_api` / `new-api` while retaining existing per-second pricing and the ultimate `dashscope.aliyuncs.com` upstream marker. Async credit settlement behavior and Wan2.6 T2V/R2V synchronous compatibility are unchanged.
...
- 2026-07-24：AI Chat 的小T画布智能体模式面向全量用户默认开启；preferences 升级到 v4，旧用户在缺少显式偏好时自动启用，同时持久化用户后续主动开关的选择。
- 2026-07-24：普通 AI Chat 的生图对话迁移到小T。手动生成直接进入小T；Auto 工具选择命中 `generateImage` 后复用已创建消息转交小T，由当前小T大脑整理提示词、选择图片模型并创建/运行画布节点，不再调用旧的 `generateImage` 直连流程。
- 2026-07-24：小T内容安全改为完全依赖 `xiaot-agent` 渠道自身判断。移除 Tanvas 前端关键词/姓名拦截与输出替换、后端 AI Guard、Agent 本地拒答和额外安全 system prompt；Tanvas 只原样调用小T，避免重复风控和策略口径不一致。
...
## [Unreleased]
### Integration
- 2026-08-21：桌面端推进到 M1 基础。新增任务级项目绑定，切换任务/项目自动关闭旧工具面；接通 `⌘K`、`⌘N`、任务重命名/删除和账号退出；插件宿主改为订阅注册表并按插件独立持久化宽度。基于 JZXZ 1.1.11 安装包静态分析，纠正首批能力定位为专业软件 MCP、本机隔离运行时、动态 Skills、本地 RAG 与 CAD/3D 交付证据；新增 `tanva.desktop-connectors` 内置插件及受控 Electron IPC，可检测、手动指定和启动 SketchUp、Rhino、Grasshopper、AutoCAD、Photoshop。新增官方 MCP TypeScript Client 驱动的 stdio Capability Host，支持绝对路径配置、真实握手、工具发现、按需 schema 查询、四级风险标注和 Main 原生逐次确认；二进制/内联/base64/超长结果不会进入 Renderer 持久化。新增真实桌面壳浏览器验收、MCP 协议夹具测试与 JZXZ/Tanva 能力映射文档。
- 2026-08-21：落地 Tanva Electron M0。Electron 使用隔离上下文、沙箱 Renderer、关闭 Node integration、受信导航与最小窗口 IPC；桌面 UI 采用任务边栏 + 嵌入式小T线程 + 统一右侧工具面。新增 v1 插件 manifest、注册表、宽度/停靠/最大化状态和错误隔离，现有 Tanva 画布以 `tanva.canvas` 第一方内置插件接入；小T收到 `flow_patch` 或执行创建/编辑演示文稿时自动打开画布。扩展列表只管理/展示插件，不提供平行能力启动入口。新增插件合同测试、独立零依赖 Electron runtime 和 electron-builder 打包链路，macOS arm64 未签名 `.app` 已实际启动验证。
...
- Flow/豆包视频分析计费与启用：补齐 Seed 2.0 Mini/Lite/Pro 在 `ark-doubao` 的模型目录、channel models 与 abilities。Pro/Lite/Mini 均以标准付费 Seedance 2.0 480P 为价格锚点，按后端探测到的视频真实时长分别以 `1/3`、`1/10`、`1/20` 计费（当前分别为 `125/3`、`12.5`、`6.25` 积分/秒，整段总价向上取整），前端媒体时长只作试算；Responses token 仅保留作上游调用审计，不参与这三个模型的用户侧扣费。旧 `60/90/120` 固定扣费不再参与豆包报价，内部 `pricingSnapshot` 只供服务端/管理员审计。
...
- Admin/RBAC: normal admins can enter the admin console and keep non-sensitive admin views such as dashboard, user/order lists, credit records, API stats/records, templates, and user/team detail views. They cannot access paid-user management, watermark whitelist, system settings, or node management, and user management hides/backend-blocks add user, delete user, credit add/deduct, and user membership operations. Full admins (`admin`) remain the highest backend role and can see/use every admin tab and operation; this is independent of team permissions.
- Admin/Users: deleting a user now clears additional user-owned records before removing the account, including WeChat login sessions, user templates, generation image cache rows, bio-auth groups, video tasks, team memberships, and teams owned by the user.
...
- Flow/APIMart: video task failures now map upstream `PUBLIC_ERROR_*` raw codes (including `PUBLIC_ERROR_UNDERSPECIFIED_ANIMAL`) to localized user-facing copy in Flow nodes while keeping the raw message in console logs.
...
### Fixed
- AI Chat/小T：画布任务终帧兼容读取 `message` 与 `data.text` 正文并按 Markdown 展示，正文下方单独显示“已完成”；仅无正文时才用“任务已完成”或“任务已停止”替换思考占位。
...
- Flow/Runtime: repeat node runs now replace stale progress timestamps at click time so Seedream/GPT Image/Banana reruns do not jump directly to or remain stuck at 95%.
...
- 修复 Hailuo H3 腾讯轮询误报成功：腾讯可能返回外层 `FINISH`，但嵌套 `AigcVideoTask` 仍带非零 `ErrCode` 和失败消息；后端现在优先识别该错误，并要求存在真实视频 URL 后才报告成功，同时给 base64 图片上传保留原始 MIME，避免无类型对象继续进入腾讯任务。
...
## 2026-09-13 邀请积分逐账户核验与充值保护（101 已部署）

- 按用户最终口径撤销“全部历史奖励视为已消费”，原额回放 784 个账户的成功消费/退款/衰减；226 个完成有证据的修正或核验，4 人共返还 258，Jayden 返还 150 后余额 47,060。116 个无新增待处理奖励，442 个历史证据未闭环不自动改账。
- 未核验历史迁移批次继续优先消费，暂缓衰减；核验后真实余量及新邀请正常衰减。新增其他非衰减批次余额保护，防止陈旧免费批次侵蚀充值余额。
