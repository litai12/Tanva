...
- 2026-08-31：恢复并收紧免费积分生命周期。免费用户签到积分仅在当前凌晨 3 点切换的签到业务日内有效，余额查询、个人消费、签到入账与每日 3 点任务都会清除跨日剩余；有效月卡、年卡及 VIP 白名单签到跳过次日整批清理，但仍归入 permanent gift 批次，仅在当前资格有效时暂停衰减，资格失效后恢复每日 50 分衰减，且签到批次优先消费。历史 VIP 签到误清积分由迁移自动补回并统一改为可衰减 gift。非有效 VIP 用户的免费签到、邀请奖励、运营赠送和受邀注册免费额度等免费积分池继续在每日 2 点按策略默认衰减 50，判定从“新用户/历史是否充值”纠正为“当前是否有有效 VIP”；`gift` 统一代表免费可衰减积分，充值本金和充值赠送统一归为不可衰减的 `recharge`。邀请奖励新增 gift lot，历史无 lot 邀请流水兼容衰减且同日幂等。
...
- 2026-08-31：Flow 的历史 `omniFlashExtVideo` 节点保留类型和 `managedModelKey=omni-flash-ext` 以兼容旧画布，但产品名与实际上游已切换为 ToAPIs `gemini_omni_flash`。节点仅保留 720P/1080P 和 4/6/8/10 秒，参考视频仍携带用户选择的输出时长。用户价按供应商成本 `x1.5` 计算并向上取整积分：720P 4/6/8/10 秒=`158/158/189/210`，1080P=`210/210/231/263`。new-api 增加同矩阵的提交前计费因子与幂等生产补丁。
