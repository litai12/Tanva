...
- Flow/HTML PPT: Run can now conservatively plan one generated visual asset when the prompt clearly asks for a hero/background/illustration image, call the image-generation route first, upload any inline result to a remote asset, and pass the generated URL into the final PPT layout prompt so it is placed into the HTML deck.
