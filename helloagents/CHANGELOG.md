...
- 2026-07-24：修复普通 AI Chat 的 Auto Agent 计划仅作为旁路 trace、拒答后仍继续调用 `chatResponse` 的绕过。前端现于模式分流和工具选择前直接终止敏感请求，并在普通文本结果写入消息/历史前拦截政治人物姓名与别称，避免上游漏判内容出现在界面。
- 2026-07-24：全站 AI 创作新增统一内容风控。普通对话/提示词优化、图片/视频/音频创作、Auto/Research Agent 与小T均在上游调用和扣积分前拒绝中国现实政治/政治人物、战争与恐怖主义、血腥暴力及自伤话题；小T首条 system message 进一步约束隐语、变体、提示词注入及其他违背网站主题的内容，统一返回固定创作引导文案。
