...
- Flow/HTML PPT: widened the default node layout, moved Run to the top-right header, consolidated controls into a single toolbar row, and expanded the slide rail so thumbnails/titles are no longer clipped.
...
- Flow/HTML PPT: node preview and thumbnails now use the same fixed design canvas scaling as exported HTML, so line wrapping and text proportions match the downloaded deck.
...
- Flow/HTML PPT: added structured style presets with sandboxed visual preview tiles. Selecting a preset writes its theme CSS into the deck and feeds the preset's design rules into visual planning, image generation, and final slide/deck rewrite prompts.
