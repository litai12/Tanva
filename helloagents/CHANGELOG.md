...
- Flow/Prompt Nodes: Prompt and Prompt Pro textareas now stay in preview mode until the node is selected, so canvas/node drag gestures over unselected prompt text no longer focus the textarea or get swallowed by `nodrag/nopan` handlers.
