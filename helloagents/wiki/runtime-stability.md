...
- Added OpenObserve `backend_error` reporting for backend exceptions:
  ...
  - when a backend exception happens after an upstream `fetch`, `backend_error` also includes `upstream_payload`, `upstream_response`, and `upstream` using the latest sanitized upstream request captured in the current request context
