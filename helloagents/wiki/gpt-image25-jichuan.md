...
## 2026-09-14 GPT Image 2 质量默认值

GPT Image 2 质量只提供 `low / medium / high`，默认 `low`。新建和切换到该型号均使用 low，稳定线路的历史 auto 或缺失质量在展示、报价和实际生成请求中归一为 low。普通线路不提供质量选择，报价和实际生成请求均省略 quality，前后端忽略旧默认值及切换线路残留的质量参数；Flare/Sunburst 继续省略质量参数。
