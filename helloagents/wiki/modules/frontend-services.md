...
- 项目内容保存是整包 JSON PUT，属于服务器压力热点；自动保存最小持久化间隔保持 60s，且只有 `content.flow.nodes` 非空时才发起自动保存。Flow 缺失或节点为空时保留 `dirty` 但不 PUT，手动保存链路不受该过滤影响。保存后的项目缩略图刷新 cooldown 保持 5 分钟，避免 100 人级在线编辑时产生 OSS/DB/遥测写入放大。
- AI/视频等影响积分的 POST 请求成功后会触发全局 `refresh-credits`，但事件源头需要做短窗口合并，避免批量生成时重复拉取 `/api/credits/balance` 和签到状态。
- `promptLibraryApi.ts` 封装官方案例分页、自定义提示词 CRUD 和常用状态；UI 不直接请求 `localhost:5175`。自定义封面由 `imageUploadService.uploadImageFile` 进入 `prompt-library/covers/`，成功取得远程 URL 后才提交提示词 API，禁止把 `blob:`、`data:` 或裸 base64 写入数据库。
