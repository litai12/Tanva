...
## 2026-09-07 聊天视频统一画布执行

- 旧聊天视频链路固定 1.5 Pro 且省略分辨率/音频/渠道，可能命中无声 720P 价格，与用户画布实际选择不一致；现已删除直接创建、轮询及退款逻辑，统一由小T创建并执行画布视频节点。
- 兼容 `generateVideo`、手动 video 模式及 Auto 选中的视频请求均转交画布，保留消息身份、时长和参考图。参考图上传后作为选中的 image 节点送入画布上下文；成功交付要求节点返回真实视频资产。
- `generate-video-provider` 对 1.5 Pro 读取 `doubaoVideo.status`，禁用/维护/未开放在积分预扣和上游调用前返回 400，阻断旧聊天客户端和旧画布绕过节点禁用。
- 验证：聊天转交/附件回归及画布执行验收测试；后端覆盖节点三种不可用状态、普通可用状态，以及旧请求和 API Key 请求均在扣费/上游提交前被拒绝。

## 2026-09-07 后台用户积分明细查询

- 用户积分详情中的细分流水独立分页，默认每页 20 条，可切换 50/100 条；总数和页数来自服务端筛选结果。
- 管理员流水接口支持 `startDate`（含）、`endDate`（不含）和 `model`（忽略大小写的模型关键词）。前端把所选结束日期转为本地次日零点，覆盖结束日全天；日期按流水 `createdAt` 筛选。
- 模型通过当前用户的关联 `ApiUsageRecord.model` 匹配，消费与退款均保留，无模型的充值/签到在指定模型时不匹配。SQL 使用参数化 EXISTS，在数据库完成筛选、计数及分页，按创建时间和 ID 倒序稳定排序。
- 查询/重置回到第一页，切换用户清空条件；请求失败展示错误，失效请求不能覆盖新查询。

## 2026-09-07 积分来源消耗排序与旧账保护
...
- consume policy：
  ...
  - 先强制签到 → 管理员手动充值 → 系统邀请 → 会员 → 单独购买的来源顺序，再在同级内使用配置排序。来源判定和旧账规则见上方 2026-09-07 说明。
- 会员 P0 最小闭环：
