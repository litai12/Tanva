...
- `research_cases` emits an additional `research_result` event with architecture case cards, source links, and image slots. When `VOLC_SEARCH_ENABLED=true`, `VolcResearchSearchService` (`VOLC_SEARCH_*`) first asks the configured text model (`VOLC_SEARCH_SUMMARY_MODEL` / `ARK_WEB_SEARCH_MODEL` / default text model) to plan concrete building/project names from the user request, treating user-specified architects/subjects as hard filters. It then runs Volcengine web search per planned project name, ranks returned sources by project/architect relevance without requiring exact Chinese-title matches, summarizes only source-backed matches into case cards, validates that card titles are project names rather than people/articles/lists, and finally runs image search from the same case list. When search is disabled, fails, or returns no matching cases/results, the backend returns an explicit no-result summary instead of unrelated static case cards.
