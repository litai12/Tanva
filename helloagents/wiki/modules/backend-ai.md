...
- Banana 文本对话打开 `enableWebSearch` 时会优先走 147 legacy 文本通道并挂载 Google Search 工具；若该通道失败，会回退到配置的文本通道，但回退通道不保证联网搜索能力。返回值会带 `metadata.webSearchEnabled/requestedWebSearchEnabled` 供前端标识实际状态。
