...
- `XiaotAgentService` 把 SSE `[DONE]` 仅视为传输结束：完整 tool-call JSON、正文、`flow_patch`、`host_tool`、`host_ui` 至少要存在一种，才允许结算固定对话积分并发送 `final/done`。工具参数截断或四类交付全部为空会使 run 显式失败，避免上游空 `finish_reason=stop` 被投影成成功。
