...
- The new-api APIMart payload builder has an `omni-flash-ext` branch: prompt is required; `image_urls` are collected from standard image fields and metadata content; only 0/1/3 images are accepted; `generation_type` is emitted only when images exist and 3-image requests require `reference`; `video_urls` are collected from reference video fields/metadata, limited to one URL, and cause `duration` to be omitted.
