...
- 已是官方格式的占位符保持不变且不会重复添加；自定义分镜的每条 `multi_prompt.prompt` 只转换其中显式出现的 `@图N`，不会把所有参考图强行注入每个镜头。越界别名（例如只有两张图时的 `@图10`）保留原文，不会错误匹配为 `@图1`。
- Kling 3.0 Omni 的视频输入必须区分 `feature`（视频参考）与 `base`（视频编辑）。UI 未持久化该选择的历史节点按 `feature` 解释；只有明确选择视频编辑时才允许发送 `base`。参考视频本身的时长不能覆盖节点选择的生成时长，例如 3 秒参考视频、6 秒输出必须继续向 ToAPI 与腾讯 VOD 提交 `duration=6`。
...
- 视频编辑的 `duration=-1` 仅是 Ark 上游协议值，不进入积分计算；后端先探测参考视频真实时长，将跟随输入的实际输出时长还原为同等时长，再按 `billingDurationSec = inputVideoDurationSec + outputDurationSec` 计费。例如输入 10 秒、输出跟随为 10 秒，按 20 秒计费；正数计费上下文与上游 DTO 保持分离。
...
- 非小T文本请求统一经 `NewApiProvider` 发送到 new-api。未声明业务标签的普通对话与 PDF 分析默认 `gpt-5.6-terra`；HTML PPT、Paper.js、图像转矢量与普通 Agent 研究默认 `gpt-5.6-luna`。工具选择仅有一个候选时在前后端双边短路，不调用模型也不扣工具选择积分；多候选时限制为已实测可用的 `gpt-5.6-luna`、`gpt-5.6-terra`，默认 Terra，并使用 `tanvas-right-*` 专用模型名固定走 Tanvas new-api 的 Right 直连渠道。Flow Text Chat 与提示词优化分别显式提交 `billingTag=text_chat` / `billingTag=prompt_optimize`，共享同一 Luna/Terra 白名单与 Terra 默认值。图片识别/分析是独立的 Gemini 三档链路，不随文本模型切换。
...
- new-api 中令牌所属分组必须存在请求模型对应的 ability。工具选择、Flow Text Chat 与提示词优化在产品及计费层使用 `gpt-5.6-luna`、`gpt-5.6-terra`，发送 new-api 前分别转换为内部专用模型 `tanvas-right-gpt-5.6-luna`、`tanvas-right-gpt-5.6-terra`。这两个模型只绑定 type 60 的 `right` channel，并由 channel model mapping 转回 Right 上游真实模型名；不得改用裸模型 ID，否则可能被其他高优先级渠道接管。图片生成的 `providerOptions` 不透传到工具选择文本请求，避免 stable 路线把 Right 文本错误切到 VIP token 分组。
- 显式 `billingTag=text_chat` / `billingTag=prompt_optimize` 在调用 Right GPT 前必须先通过 new-api 调用 `deepseek-v4-flash-260425`。DeepSeek 只返回版本化 JSON 判定 `allowed/politicalViolation/sensitiveTopic/reason`；服务端做类型、非空与逻辑一致性校验，只有 `allowed=true` 且另外两项均为 `false` 才能继续。审核拒绝返回明确的客户端错误；审核上游失败、空正文、非法 JSON 或字段矛盾返回服务不可用。所有未放行分支都不得创建 Right GPT 请求，并沿现有计费事务回滚。
- Flow Text Chat 与提示词优化走普通同步 OpenAI Chat JSON，不发送小T专用的 `mode`、`executionToolPolicy` 或临时 agent user，也不读取小T session、记忆、画布和 durable turn。小T画布回合继续使用自身独立 facade 与宿主上下文协议，两条链路不得复用。
...
- `POST /api/ai/text-chat` 的同步 JSON 终态合同为 `HTTP 200 + 无 error 信封 + 非空 assistant 正文`。显式 `text_chat` / `prompt_optimize` 同样执行这条合同，不再进入 agent SSE 续接；new-api 返回 `202 pending`、内嵌 OpenAI error或空正文时必须失败，Controller 与计费包装器继续执行终态正文校验并退款。图片、视频等异步任务不受该文本终态合同影响。
- 视频分析由 `resolveVideoAnalysisModel` 校验节点显式模型；默认豆包 Seed 2.0 Lite，也支持豆包 Mini/Pro 与 Gemini 三档。小T继续开放自己的独立大脑模型；Flow Text Chat / Prompt Optimizer 只显示 Right 已实测成功的 GPT-5.6 Luna 和 GPT-5.6 Terra，不显示小T前缀或 Right 实际返回 `model not found` 的 DeepSeek。
- 无真实调用验证：`npm run verify:new-api-text-routing` mock `fetch`，覆盖 GPT-5.6 文本、联网工具与 thinking 字段、Gemini 图像分析、统一 new-api URL/鉴权，以及只有 tc-api key 但缺少 `NEW_API_KEY` 时显式失败；`npm run test:new-api-text-terminal` 覆盖 `202 error/pending`、`200 error`、空正文和正常 Chat/Responses 终态；`npm run test:text-chat-terminal-billing` 验证空正文绝不成功结算并触发用量失败与退款；`npm run test:text-chat-right-routing` 验证两个业务入口均严格按 DeepSeek→Right 顺序执行；`npm run test:tool-selection-right-routing` 验证唯一候选零调用/零扣费及多候选 Terra/Luna 专用 Right 路由。
...
- Seed 2.0 Pro/Lite/Mini 的产品计费均锚定标准付费 Seedance 2.0 480P 的 `1.25 元/秒`，比例分别为 `1/3`、`1/10`、`1/20`，对应 `125/3`、`12.5`、`6.25` 积分/秒。定价器先用后端识别的完整视频时长计算精确总价，再向上取整一次；前端可用媒体 metadata 调用 `/api/credits/preview` 展示预估，但实扣不信任客户端时长。Responses token usage 仍写入内部 `pricingSnapshot` 供服务端与管理员核对上游调用，不参与这三个模型的用户侧视频分析扣费；公开 `billing` 对按时长模型返回 `billingMode/creditsCharged/durationSec`，用户 usage 会剔除内部快照。
...
- Seedance/new-api 的 `doubao-video` 异步任务不再只依赖浏览器轮询：`NewApiVideoTaskReconciliationService` 每分钟按已持久化 `newapi:` taskId 查询上游，终态幂等确认或退款，并把成功后转存的 Tanva OSS 视频地址写回用量参数；项目重载时优先复用该地址，避免 new-api 临时 URL 过期导致画布无法回填。确认成功时还会以 `taskId` 为幂等键写入用户全局历史（`mediaType=video`），因此浏览器关闭、刷新或前端进程丢失后，完成视频仍可在“全局历史”查找。
- 所有由 `GenerationTaskService` 持久化的后端视频任务，在写入 `succeeded + result.videoUrl` 时也会自动写入同一份全局历史；该记录只保存视频/封面的远程 URL，不保存 `data:`、`blob:` 或 base64。
...
- `POST /api/ai/analyze-video` 的 Gemini 兼容路径仍保留既有固定档位；豆包 Seed 2.0 Pro/Lite/Mini 均按真实视频时长与 Seedance 2.0 480P 的产品比例计费，分别为 `1/3`、`1/10`、`1/20`。旧 normal `60/90/120`、stable `80/120/160` 不得再用于豆包报价或扣费。
...
- 部署注册与支持范围见 `frontend/docs/Wan3.0接入说明.md`；支持 `input.media` 中的 first_frame/last_frame/reference_image/reference_video，校验互斥组合、数量和托管 URL，视频探测实际时长后再扣费；提示词/媒体至少一项。

- Wan3.0 Compose 自动注册：`new-api-patch` 扫描 `2026-09-07/001-add-wan3-0-video.sql`，复制启用的 type=17 Wan2.7 ability 并写入每秒 `ModelPrice=0.45`；保留人工禁用和其他模型价格。无源渠道时事务回滚且不记录已执行，配置后可重跑；缓存按同步周期刷新。
