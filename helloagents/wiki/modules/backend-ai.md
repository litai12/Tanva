...
- new-api 的 `default` 分组必须存在已启用的普通 `gpt-5.4`、`gpt-5.6-luna` abilities；小T专属 `xiaot-agent-gpt-5-4|5-5` facade 不能承载这些普通文本请求。网关缺 ability 时在 new-api 管理后台补渠道、上游 base URL 与 key，不在 Tanva 后端增加直连凭据或 fallback。
