...
- 非小T文本请求统一经 `NewApiProvider` 发送到 new-api。未声明业务标签的普通对话、工具选择和 PDF 分析仍按各自模型链路处理；HTML PPT、Paper.js、图像转矢量与普通 Agent 研究默认 `gpt-5.6-luna`。Flow Text Chat 与提示词优化分别显式提交 `billingTag=text_chat` / `billingTag=prompt_optimize`，共同限制为已实测可用的 `gpt-5.6-luna`、`gpt-5.6-terra`，默认 Luna，并忽略图片 provider，固定走 Tanvas new-api 的 Right 直连渠道。图片识别/分析是独立的 Gemini 三档链路，不随文本模型切换。
...
- new-api 中令牌所属分组必须存在请求模型对应的 ability。提示词优化在产品与计费层使用 `gpt-5.6-luna`、`gpt-5.6-terra`，发送 new-api 前分别转换为内部专用模型 `tanvas-right-gpt-5.6-luna`、`tanvas-right-gpt-5.6-terra`。这两个模型只绑定 type 60 的 `right` channel，并由 channel model mapping 转回 Right 上游真实模型名；不得改用裸模型 ID，否则可能被其他高优先级渠道接管。
...
- 无真实调用验证：`npm run verify:new-api-text-routing` mock `fetch`，覆盖 GPT-5.4 文本、联网工具与 thinking 字段、GPT-5.6 Luna 图像分析、统一 new-api URL/鉴权，以及只有 tc-api key 但缺少 `NEW_API_KEY` 时显式失败；`npm run test:new-api-text-terminal` 覆盖 `202 error/pending`、`200 error`、空正文和正常 Chat/Responses 终态；`npm run test:text-chat-terminal-billing` 验证空正文绝不成功结算并触发用量失败与退款；`npm run test:text-chat-right-routing` 验证两个业务入口均严格按 DeepSeek→Right 顺序执行，拒绝与不可验证判定时 Right 调用数为零。
