...
- 豆包计费使用 Responses 返回的真实 `input_tokens` / `output_tokens` / `input_tokens_details.cached_tokens` / `audio_tokens`。定价器按实际输入长度选择火山官方 `0–32K`、`32–128K`、`128–256K` 常规在线推理价格，用户价为 `官方成本 × 1.5`，再按 `100 积分 = 1 元` 换算并向上取整；API usage 的 `pricingSnapshot` 保留 token、档位、官方成本、倍率、人民币售价和实际积分。该快照只供服务端与管理员审计：分析接口的公开 `billing` 仅返回 `billingMode/creditsCharged`，用户 `/api/credits/usage` 也会剔除 `pricingSnapshot`，公开 Credits catalog 与 NodeConfig description 同样只写“按实际用量结算”。节点仅显示“按量计费”与上次实扣，不向用户披露供应商成本、倍率或分档单价。
...
- 小T大脑使用专属门面模型：Fast=`xiaot-agent-gpt-5-4`、Pro=`xiaot-agent-gpt-5-5`、Ultra=`xiaot-agent-gpt-5-6-luna`，默认与非法值回退为 Fast。Tanva new-api 的 `xiaot-agent` 渠道通过 `model_mapping` 翻译成 TapCanvas facade 的 `gpt-5.4|gpt-5.5|gpt-5.6-luna`；不能直接请求裸模型名，否则会绕过小T facade。
