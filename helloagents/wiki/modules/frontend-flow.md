...
- Provider-video nodes (`Seedance/Kling/Vidu` family) now resolve active Prompt `@` image mentions at run time and prepend them to connected image references. Seedance mode validation counts those mentions as image references, so `@`-only Prompt inputs can drive Seedance reference-image runs without separate image-handle wiring. Video runs also append a final reference mapping for `图1`/`图2` and mention tokens, and clear the previous preview before starting so stale output is not mistaken for the new run.
...
- `FlowOverlay` 在运行 `generate` / `generate4` / `generatePro` / `generatePro4` 以及 provider-video 节点（Seedance/Kling/Vidu 系列）时，会从连接的 Prompt 节点读取仍存在于文本中的 image mentions，并与图片输入边合并、去重、按节点参考图上限截断；项目库/个人库只保存并传递远程 URL/路径引用，不把 inline 图片写入设计 JSON。
