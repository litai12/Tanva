...
- 选择至少两个节点时，画布底部显示多选工具栏，提供左对齐、水平居中、右对齐、顶部对齐、垂直居中和底部对齐，以及现有打组入口。
- 对齐基准为当前选区包围盒，优先采用 React Flow 实际测量尺寸；按指定轴对齐，另一轴保留原视觉顺序并补足至少 40 的间距，避免节点互相叠压。未选中节点不参与布局，选中组的成员整体平移，父子相对坐标不重复移动。
- 协作锁定节点（包括成员被锁的整组）保持原位。位置变更走现有项目同步、历史记录和协作补丁通道，不修改图片资产引用。
- 工具栏按聊天浮层的实时位置选取可用矩形：紧凑聊天框时上移，右侧展开时移入左侧画布；聊天拖动、显隐和窗口缩放后更新，窄屏允许工具栏内部横向滚动。聊天最大化没有可用区域时隐藏工具栏。
- 对齐后按选中节点包围盒适配可用视野，预留工具栏和边距，沿 CanvasStore 的视口同步通道缩小/平移（最小缩放沿用 10%，不主动放大）。重复对齐无坐标变化时不添加历史记录。
- 回归命令：`node --test frontend/src/utils/flowSelectionAlignment.test.ts frontend/src/utils/flowSelectionViewport.test.ts`；启动 Vite 后，在 frontend 运行 `node scripts/verifyFlowSelectionToolbar.mjs`，验证真实组件的紧凑聊天、展开侧栏、窄窗、聊天关闭与对齐按钮点击。

## 2026-09-04 Seedance 参考媒体时长预检

- Seedance 2.0/2.5 的 Flow 运行路径不再在浏览器读取或拦截参考视频、参考音频的单条/总时长。前端只保留素材数量、任务类型与输出规格等结构性校验，并将原始远程引用提交给上游；参考视频真实时长仍可由后端在计费边界探测，最终可用范围由 new-api/Seedance 返回。

## 2026-08-24 Kling O3 图片引用

- Kling O3 节点继续让用户用 `@图N` 指代按去重后顺序排列的参考图。运行请求不再要求用户了解供应商模板语法：后端会转换为 Kling Omni 的 `<<<image_N>>>`，纯连线图片也会自动获得对应占位符，因此“图片参考”模式不会因只连线、未手写 `@图N` 而被上游拒绝。
...
- 导出不会把 data/blob/base64 写回设计 JSON。远程图片需支持浏览器 CORS；无法读取时明确提示先上传到 Tanva 项目库。导出运行时是动态 chunk，不增加主入口的 PPTX 引擎体积。
...
- 普通线路：Fast=`20`、Pro 1K/2K/4K=`60/70/85`、Nano Banana 2 0.5K/1K/2K/4K=`40/40/50/70`。尊享线路：Fast=`40`、Pro=`130/130/240`、Nano Banana 2=`45/65/100/155`。
- GPT Image 2 普通 1K/2K/4K=`20/30/40`；尊享 Low=`30/40/50`、Medium=`60/120/190`、High=`230/460/760`，参考图仍追加 `10 积分/张`。
- 顶部线路切换器与 AI 设置只显示普通、尊享。preferences v7 将历史 `ultra/beqlee` 迁回普通，两个图片请求封装也在发送边界执行相同归一化，防止旧页面继续使用暂停的极速渠道。
...
- 视频编辑隐藏宽高比选择并固定 `adaptive`，输出时长使用 Ark 协议值 `-1` 跟随 4–30 秒输入视频；价格预览先把实际输出还原为输入视频时长，再按“输入视频时长 + 输出视频时长”求和。视频延长同样固定 `adaptive`，保留用户选择的 4–30 秒输出时长，并按输入与输出时长之和计费。
...
- Hailuo 连线接纳、模式切换时的旧连线裁剪、Run 前校验和 new-api 预扣校验共同限制参考素材数量；目录请求暂时失败时，已保存节点规格仍作为前端展示与连线校验回退。
- Hailuo Run 统计图片时会先合并物理图片连线与 Prompt `@` 图片引用并按实际渲染资源去重；同一张图同时连线和被 `@` 引用只计 1 张，避免 8 张图片被重复算成 16 张而误触发 9 张上限。
...
- Flow Prompt Optimizer 与 Flow Text Chat 标题栏使用同一组 Right 直连模型胶囊：`GPT-5.6 Luna`、`GPT-5.6 Terra`。Prompt Optimizer 的 `promptOptimizationModel` 与 Text Chat 的 `textChatModel` 分别持久化选择，但共享同一选项和同步终态协议；旧、非法、缺失以及已确认 Right 不支持的 DeepSeek 生成模型值迁移到 Terra。Text Chat 显式提交 `billingTag=text_chat`，保持文字对话计费产品，界面不得显示“小T”前缀。两类请求均由后端在生成前执行 DeepSeek V4 Flash 安全审核；前端不自行做关键词审查，也不在拒绝后改用其他模型。
...
- Every `runNode` invocation now replaces any previous transient progress timestamp at click time; repeat runs therefore restart from the beginning instead of inheriting a stale 95% clock.
...
- Flow 节点类型为 `directorConsole`，节点卡片通过 `DirectorConsoleModal` 打开 3D 编辑态；已有 `scene` / `timeline` 数据结构继续作为持久化真相源。
- 新建导演台的默认 scene 包含 `机位1` 与 `角色A`，分别使用稳定 ID `default-camera-1` / `default-character-1`；旧节点只要已有 scene 就不会被默认值覆盖。
- 编辑态顶部提供 `导演视角 / 机位视角`，直接写回 `DirectorConsoleData.activeViewpoint`。机位视角替代旧的右栏重复 POV 小窗，右栏专注显示当前角色或机位属性。
- 底部工具条是悬浮深色胶囊，并提供 `场景编辑 / 动画时间轴` 本地 UI 模式。时间轴只在动画模式挂载，但仍读取和写回原 `scene.timeline`，模式切换不会清空镜头或角色动作。
- 截图、灰模样片和时间线合成沿用现有上传链路；运行时 dataURL 不进入 node data，发送到画布前上传并只持久化远程 URL/路径。

# Flow 自动整理选区语义（2026-08-30）
