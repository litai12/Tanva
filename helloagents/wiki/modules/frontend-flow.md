...
`Muti Gen` 节点固定输出 4 张图，不再暴露可编辑的数量配置，避免 UI 配置与实际执行次数分叉�?
运行时 4 个槽位并发请求，哪个槽先完成就先写回对应输出；槽位 loading 以该槽是否已有图片或错误为准，不依赖顺序完成。
...
- **裁切预览性能**：`Image`/`ImageSplit` 裁切预览共享同一源图加载缓存，并按节点显示尺寸绘制 canvas；持久化仍保持“原图引用 + crop 参数”，避免多切片节点可见时重复解码/高清 canvas 绘制导致 FPS 下滑。
