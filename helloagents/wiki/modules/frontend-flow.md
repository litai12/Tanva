...
- `Seedance 2.0` 节点现已独立支持模式化输入：`文生视频 / 首帧 / 首尾帧 / 全能参考 / 智能多帧 / 视频参考 / 图片+音频 / 图片+视频 / 视频+音频 / 图片+视频+音频`。节点会按所选模式动态隐藏无关句柄，并在运行时校验 `1-9` 图参考、`2-10` 智能多帧、`4-15s` 时长、`21:9/16:9/4:3/1:1/3:4/9:16` 比例，以及 `Seedance 2.0 / Seedance 2.0 Fast` 模型切换。
