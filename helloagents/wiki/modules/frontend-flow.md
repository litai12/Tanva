...
- When the prompt clearly asks for a hero image, background, illustration, poster, or similar generated visual, Run first asks the text model for a one-asset visual plan, calls the image-generation route with optional upstream reference images, uploads inline image output to a remote asset, and then passes that generated URL into the final slide/deck rewrite prompt for actual placement.
- Style presets are stored as structured data (`htmlPptStylePresets.ts`) with theme CSS, prompt guidance, image-generation guidance, colors, tags, and sandboxed preview slides. The node's Style panel renders the preset previews in inert iframes; applying a preset updates deck `themeCss` and persists `stylePresetKey` for subsequent AI runs.
- The Style panel also includes a `Bold 34` tab. `htmlPptBoldTemplates.ts` maps the 34 `zarazhangrui/beautiful-html-templates` entries to Tanva-safe structured metadata, while `htmlPptBeautifulTemplateDecks.ts` stores converted static starter decks from the upstream `template.html` files. Gallery tiles render the README-selected real template pages, and applying one replaces the current deck with the converted 1920x1080 starter slides.
...
- The node preview iframe and slide thumbnails use the same fixed design canvas scaling, measured from the preview frame, so preview wrapping matches the exported HTML deck.
