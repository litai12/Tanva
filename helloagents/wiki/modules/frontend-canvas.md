...
- `图片拓展` 使用 `Expand` 图标，和 `裁切` 的 `Crop` 图标保持区分。
- `ImageContainer` 的“改文字”工具分两步执行：OCR 识别固定走 Banana `normal` 路线，避免全局尊享 `stable` 路线把识别请求送入腾讯 text 通道后长时间卡在“识别中”；最终图片文字修改仍显式携带当前 `bananaImageRoute`，尊享路线下继续走尊享改图。文字修改结果按原图 `bounds` 覆盖放置在原图上方，不参与右侧智能排版。
- `高清放大` 点击后会在原图右侧创建同显示尺寸的占位框；4K 结果完成后通过 `triggerQuickImageUpload` 上传并落到画布占位位置，不再触发浏览器直接下载。
- 扩图选择器只在选择层打开期间锁定画布滚轮/触控板交互；用户确认发送后选择层关闭并立即释放锁，后续异步生成不阻塞画布移动。扩图合成画布使用红色蒙版底，并通过预设提示词要求模型填充红色蒙版区域；扩图等待占位框保留扫光反馈，但不显示进度条和百分比。
...
## 库面板（右侧）
- 顶部提供三标签：`全局历史`、`项目库` 与 `个人素材`。
- `个人素材` 维持原逻辑：来自 `personalLibraryStore + personalLibraryApi`，支持上传/删除/详情/发送到画板。
- `全局历史` 与 `项目库` 在库面板内独立拉取，支持搜索、类型筛选、页码分页（`1 2 ... N`）、点击发送或拖拽到画板。
- 详情浮层固定显示在库面板左侧同一位置，内容超出可用高度时在浮层内部滚动，避免选择下方素材时底部信息被截断。
