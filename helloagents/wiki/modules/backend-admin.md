...
- Default monitored nodes include NANO BANANA GEMINI IMAGE GENERATION / EDIT / BLEND, NANO BANANA GEMINI IMAGE ANALYSIS, GPT-IMAGE-2 IMAGE GENERATION, DOUBAO SEEDREAM 5.0 IMAGE GENERATION, MIDJOURNEY V7 / V8 / NIJI 7 IMAGE GENERATION, GEMINI AI TEXT CHAT, GEMINI PROMPT OPTIMIZE / STORYBOARD TEXT, SEEDANCE 1.5 / SEEDANCE 2.0 / SEEDANCE 2.5 / SEED 2.0 VIDEO, MINIMAX H3 VIDEO, KLING 2.6 / KLING 3.0 / KLING O1-O3 VIDEO, VIDU Q2 / VIDU Q3 VIDEO, WAN 2.6 / WAN 2.7 VIDEO, HAPPYHORSE 1.0 R2V VIDEO, OMNI FLASH EXT VIDEO, GEMINI VIDEO ANALYSIS, SEED3D / 2D-TO-3D MODEL GENERATION, and MINIMAX / TENCENT AUDIO GENERATION.
- Fast/Pro/Ultra or version variants are grouped into the same model node where they share the same product family; `channels[]` keeps provider/channel breakdowns separately. Seedance grouping is limited to the Seedance/Seed video node family (including Seedance 2.5), while MiniMax H3, Kling, Seed3D/2D-to-3D, Wan, Vidu, and HappyHorse are resolved before the Seedance fallback. H3 matches `hailuo` service records and the managed/model key `hailuo-h3`.
...
- 系统设置 `login_notice` 保存维护公告配置，包含标题、富文本正文、媒体、按钮、每用户最大展示次数、生效/结束时间和可见人员。
- `GET /api/settings/login-notice` 需要登录态；后端按当前用户判断时间与受众，并通过 `AnnouncementView` 原子记录展示次数。系统设置的 `updatedAt` 作为公告版本，新保存的公告会自然获得新的展示计数周期。
- 指定人员支持用户 ID、手机号或邮箱匹配。正文图片只能使用 HTTP(S) 或站内绝对路径；后台上传会先写入 OSS，再把远程 URL 放入富文本。

## SD2 独立使用白名单（2026-09-07）

- 统一白名单新增 `seedance2AccessWhitelist` 开关及“SD2 使用权限”标签，支持仅开此项的新增、查询、编辑、移除。默认关闭，不自动修改已有用户权益。
- SD2 开关覆盖当前统一 Seedance 2.x 访问入口（2.0 / 2.5 及其子型号），前端继续通过原访问接口判断可用性，后端提交时同级鉴权。它只开放模型使用，生成仍按正常规则扣费。
- 不授予最高档年卡、去水印、充值加赠、会员签到或积分保留。普通用户签到基础 50 积分，未用余额按原规则在次业务日 3:00 清除；已有有效会员或完整 VIP 白名单仍保留其原有权益。
- 上线前执行迁移 `backend/prisma/migrations/202609070001_add_seedance2_access_whitelist/migration.sql` 并生成 Prisma Client。回归入口：后端 `npm run test:seedance2-whitelist`。
