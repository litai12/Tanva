...
- 小T画布任务的聊天占位文案只在运行中显示；终帧正文兼容读取 `message` 与 `data.text`，并继续通过 AI Chat 的 Markdown 渲染器展示。只要本轮存在图片生成节点，必须等每个预期图片节点产生真实 HTTP(S) URL 后才显示“已完成”，并将这些 URL 作为 `media` 卡写入同一消息；facade 的等待文案或命令发出不能覆盖宿主事实。节点执行失败、连线失败或成功节点缺少真实 URL 时消息显式进入失败终态并保留画布节点状态。`verifyXiaotTurnDelivery` 还会拒绝正文、patch、宿主工具和 UI 卡全部为空的回合，因此传输层正常结束不再自动生成“任务已完成”。
