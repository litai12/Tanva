...
- AI Chat Auto 模式会并行创建 `/api/agent/runs` 规划 trace，并把 SSE 事件归并到当前 AI 占位消息的 `metadata.agentTrace`；实际工具执行仍走现有 `processUserInput` / `executeProcessFlow` 链路。
...
- 同项目内的撤销/重做虽然复用 `hydrate()` 恢复内容快照，但必须传 `preserveProjectViewReady` 保留当前 ready 状态；否则 `App` 会把一次普通 undo/redo 误判为项目重新加载并显示全屏加载层。
