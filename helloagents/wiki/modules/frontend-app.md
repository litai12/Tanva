...
- 微信公众号登录二维码创建失败后保持错误态，不再由 effect 自动重试；登录页和登录弹窗均只允许用户主动刷新，每次尝试后显示 5 秒倒计时并禁止重复点击。前端以 localStorage 中的随机 visitor ID 配合后端执行匿名用户限流，同一页面内并发创建请求会复用一个 in-flight Promise。
...
- 当前无有效月卡、年卡或 VIP 白名单的用户展示当日签到积分到期提示，文案明确在下一个凌晨 `3:00` 业务日边界失效并显示具体时间；会员/白名单资格失效后，历史跨业务日签到余额会即时清理。
...
- `MembershipPanel` 已移除最高档年卡“积分充值 8 折”权益。充值价格对所有用户不变；普通用户套餐卡显示基础积分（到账 100%），有效最高档年卡会员与开启对应权益的白名单用户显示到账 120% 及永久赠送明细。创建订单仍只提交基础积分，资格和最终到账由服务端固化。
- 管理后台将旧“水印白名单”抽象为“白名单”，列表以 Tag 展示并可独立配置“去水印 / 最高档年卡权益 / 充值到账 120%”。最高档年卡权益白名单在会员页标明“不发周期积分”。
- `MembershipPanel` 的标准版套餐卡片不显示王冠图标，权益列表不再重复展示“基础月卡积分：500”首行。
- `MembershipPanel` 会识别当前订阅是否为旧价格版本月付套餐，并允许同档或更高档当前套餐进入服务端结算预览，不再因旧套餐不在可售目录而把同档新版误锁为“仅支持升级”。确认区明确提示旧会员额度清零、新周期从付款时刻起算且独立充值积分保留。
- `PaymentPanel`（`frontend/src/components/payment/PaymentPanel.tsx`）核心交互文案已接入 `useLocaleText`（订单状态、筛选、支付提示、二维码状态、手动核对按钮）。
- `PaymentPanel` 仅在当前登录用户为超级管理员（`role = admin`）时展示“自定义积分充值”入口与编辑区；普通用户及 `normal_admin` 仅展示固定充值档位。
...
- 推广激励分组使用 `frontend/src/components/ReferralRewards.tsx`：签到成功后直接采用接口返回的连续签到结果并触发余额刷新，不重新加载整个面板、不显示“签到中”加载文案，也不弹出成功确认框；失败信息以内联提示显示。
...
- 管理后台“系统设置 → 维护公告”复用登录提醒富文本编辑器，支持标题、文字样式、正文远程图片、顶部媒体、按钮、展示次数、时间范围及指定人员。
- 公告正文允许纯图片内容。图片通过 OSS 上传并只持久化远程 URL，禁止保存 `data:`、`blob:` 或裸 base64。
- `/app` 的公告弹窗从带登录态的 `/api/settings/login-notice` 获取；是否仍可展示由后端计数决定，前端不再使用 localStorage 按登录次数去重。
