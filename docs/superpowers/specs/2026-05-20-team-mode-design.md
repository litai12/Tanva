...
**关键设计决策：**
- **每个用户都是一个团队**：注册时自动创建"个人团队"（`isPersonal=true`，`maxSeats=1`，不可增减，不可解散）
- **X-Team-Id 统一路由**：所有积分消费均通过 `X-Team-Id` 请求头指定团队，个人团队也不例外；前端登录后默认将 activeTeamId 设为个人团队 ID
- 用户可加入**多个真实团队**（有别于 TapCanvas 的单团队约束）
- 团队积分**完全独立**于旧个人积分模型（复用 CreditLot 有效期机制；现有 MembershipPlan 续期积分改为写入个人团队积分池）
- 实时协作使用 **SSE**（不是 WebSocket）
- 团队套餐与个人会员套餐**完全独立**；个人团队无需购买团队套餐，由个人会员续期驱动
...
model Team {
  ...
  name        String
  ownerId     String
  isPersonal  Boolean  @default(false)  // true = 用户个人团队，注册时自动创建
  maxSeats    Int      @default(10)     // 个人团队固定为 1，不可修改
  status      String   @default("active") // active | dissolved（个人团队永远 active）
  ...
  @@index([ownerId, isPersonal]) // 快速查找用户个人团队
}
...
### team-core
```
# 注册时由 auth.service.ts 内部调用（非对外接口）：
INTERNAL createPersonalTeam(userId)          用户注册后自动创建个人团队（isPersonal=true, maxSeats=1）

POST   /teams                               创建真实团队（isPersonal=false）
GET    /teams                               获取我的所有团队（含个人团队）
...
**所有积分消费均通过 `X-Team-Id` 请求头路由到对应团队积分池。**

- 前端登录后，默认 `activeTeamId = 用户个人团队 ID`（从 `/teams` 接口返回中取 `isPersonal=true` 的那条）
- 切换到真实团队时，`activeTeamId` 更新为该团队 ID
- 后端 AI 模块：若请求头携带合法 `X-Team-Id` 且用户为成员 → 走团队积分扣减；否则拒绝请求（不回退到旧个人积分，避免双路径维护成本）
- 现有 `CreditAccount` 模型**逐步废弃**：个人会员续期积分改写入个人团队的 `TeamCreditAccount`，旧账户余额在迁移脚本中一次性转移

**个人团队不变性约束：**
- `isPersonal=true` 的团队：`maxSeats` 固定为 1，接口层拒绝修改
- 不可邀请成员（邀请接口检查 `isPersonal` 直接返回 403）
- 不可解散（解散接口检查 `isPersonal` 直接返回 403）
- 不可购买团队套餐（订阅接口检查 `isPersonal` 直接返回 403）
- 名称显示为"我的工作区"，不可更名
...
DELETE /teams/:teamId（owner only）
  前置检查：
    - isPersonal=true → 403 cannot_dissolve_personal_team
...
| 11 | **个人团队被误操作** | 攻击者或 bug 对 isPersonal=true 的团队发起邀请/解散/修改席位请求 | 所有团队写操作的 Guard/Service 层统一检查 isPersonal，违规返回 403 |
| 12 | **注册时个人团队创建失败导致账户不可用** | auth.service 注册事务中个人团队创建抛异常，用户数据写入但无团队 | 注册事务包含 createPersonalTeam，任何子步骤失败回滚整个注册 |
