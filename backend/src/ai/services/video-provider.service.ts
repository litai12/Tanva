...
export class VideoProviderService {
  ...
  private readonly apiKeys = {
    ...
    'doubao': process.env.DOUBAO_API_KEY || '04fe24c5-2a7f-40c4-a1a8-2b1bbf1c3548',
  };

  /**
   * 创建生成任务
   */
  async generateVideo(options: VideoProviderRequestDto): Promise<VideoGenerationResult> {
    ...
    this.logger.log(`🎬 视频生成任务创建: provider=${provider}, prompt=${options.prompt.substring(0, 50)}...`);
   ...
   * 查询任务状态
   */
  async queryTask(provider: 'kling' | 'vidu' | 'doubao', taskId: string): Promise<{ status: string; videoUrl?: string; thumbnailUrl?: string }> {
    const apiKey = this.apiKeys[provider];
   ...
   * 豆包 Seedance 视频生成
   */
  private async generateDoubao(options: VideoProviderRequestDto, apiKey: string): Promise<VideoGenerationResult> {
  ...
  private async queryDoubao(taskId: string, apiKey: string) {
    try {
      ...
      const data = await response.json();
      this.logger.log(`🔍 豆包任务状态查询: taskId=${taskId}, status=${data.status}`);
      ...
      if (data.status === 'failed') {
        this.logger.error(`❌ 豆包任务失败: taskId=${taskId}, error=${JSON.stringify(data.error || data.reason || data)}`);
        return { 
          status: 'failed',
      ...
      this.logger.error(`❌ 豆包查询异常: taskId=${taskId}, error=${error instanceof Error ? error.message : error}`);
      throw error;
    }
  }
   ...
   * 可灵 Kling 视频生成
   */
  private async generateKling(options: VideoProviderRequestDto, apiKey: string): Promise<VideoGenerationResult> {
    const isImageToVideo = options.referenceImages && options.referenceImages.length > 0;
    ...
    if (!response.ok) {
      const error = await response.json().catch(() => ({}));
      this.logger.error(`❌ Kling 生成失败: HTTP ${response.status}, error=${JSON.stringify(error)}`);
      throw new Error(error.error?.message || error.message || `HTTP ${response.status}`);
    }
  ...
  private async queryKling(taskId: string, apiKey: string) {
    try {
      ...
      // 我们先尝试 text2video 路径
      let response = await fetch(`https://models.kapon.cloud/kling/v1/videos/text2video/${taskId}`, {
        headers: { 'Authorization': `Bearer ${apiKey}` },
      });
      ...
      if (!data.data || data.code !== 0) {
        response = await fetch(`https://models.kapon.cloud/kling/v1/videos/image2video/${taskId}`, {
          headers: { 'Authorization': `Bearer ${apiKey}` },
        });
        data = await response.json().catch(() => ({}));
      }

      this.logger.log(`🔍 Kling 任务状态查询: taskId=${taskId}, status=${data.data?.task_status}`);
      ...
      if (data.data?.task_status === 'failed') {
        this.logger.error(`❌ Kling 任务失败: taskId=${taskId}, error=${JSON.stringify(data.data.task_result || data)}`);
        return {
          status: 'failed',
      ...
      this.logger.error(`❌ Kling 查询异常: taskId=${taskId}, error=${error instanceof Error ? error.message : error}`);
      throw error;
    }
  }
   ...
   * Vidu 视频生成
   */
  private async generateVidu(options: VideoProviderRequestDto, apiKey: string): Promise<VideoGenerationResult> {
    const isImageToVideo = options.referenceImages && options.referenceImages.length > 0;
    const endpoint = isImageToVideo 
    ...
    if (!response.ok) {
      const error = await response.json().catch(() => ({}));
      this.logger.error(`❌ Vidu 生成失败: HTTP ${response.status}, error=${JSON.stringify(error)}`);
      throw new Error(error.error?.message || error.message || `HTTP ${response.status}`);
    }
  ...
  private async queryVidu(taskId: string, apiKey: string) {
    try {
      const response = await fetch(`https://models.kapon.cloud/vidu/ent/v2/tasks/${taskId}/creations`, {
        headers: { 'Authorization': `Bearer ${apiKey}` },
      });
      const data = await response.json();
      
      this.logger.log(`🔍 Vidu 任务状态查询: taskId=${taskId}, state=${data.state}`);
      ...
      if (data.state === 'failed') {
        this.logger.error(`❌ Vidu 任务失败: taskId=${taskId}, error=${JSON.stringify(data.error || data)}`);
        return {
          status: 'failed',
      ...
      this.logger.error(`❌ Vidu 查询异常: taskId=${taskId}, error=${error instanceof Error ? error.message : error}`);
      throw error;
    }
  }
}
