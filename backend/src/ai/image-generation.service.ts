...
5. Notable details`;
  ...
  /**
   * 流式生成文本响应 - 返回 AsyncGenerator 用于 SSE
   */
  async *generateTextResponseStream(request: TextChatRequest): AsyncGenerator<string, void, unknown> {
    this.logger.log(`Generating streaming text response for prompt: ${request.prompt.substring(0, 50)}...`);
    ...
    if (request.enableWebSearch) {
      apiConfig.tools = [{ googleSearch: {} }];
      this.logger.debug('Web search tool enabled for streaming');
    }
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      this.logger.error(`Streaming text generation failed: ${message}`);
      throw error;
    }
  }

  /**
   * 流式分析图片 - 返回 AsyncGenerator 用于 SSE
   */
  async *analyzeImageStream(request: AnalyzeImageRequest): AsyncGenerator<string, void, unknown> {
    this.logger.log(`Analyzing image (streaming) with prompt: ${request.prompt?.substring(0, 50) || 'full analysis'}...`);

    const client = this.getClient(request.customApiKey);
    const model = request.model || 'gemini-2.5-flash';
    const analysisPrompt = request.prompt || '请详细分析这张图片的内容、风格、构图等特点';
    const finalPrompt = `Please respond in Chinese:\n\n${analysisPrompt}`;

    // 解析 base64 图片
    let imageData = request.sourceImage;
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      this.logger.error(`Streaming image analysis failed: ${message}`);
      throw error;
    }
  }
