...
import { UnifiedChatDto, ChatMode, ChatTool, UnifiedChatResponse, SSEEventData } from './dto/unified-chat.dto';
...
export class AiController {
  ...
  /**
   * 统一 Chat 接口 - 整合工具选择和执行
   * 前端只需调用这一个接口，后端自动判断意图并执行
   */
  @Post('chat')
  async unifiedChat(@Body() dto: UnifiedChatDto, @Req() req: any): Promise<UnifiedChatResponse> {
    const startTime = Date.now();
    this.logger.log('🚀 Unified chat request:', {
      prompt: dto.prompt ? dto.prompt.substring(0, 50) + '...' : '(empty)',
      mode: dto.mode || 'auto',
      hasImages: !!dto.attachments?.images?.length,
      imageCount: dto.attachments?.images?.length || 0,
      hasPdf: !!dto.attachments?.pdf,
      aiProvider: dto.aiProvider,
    });

    try {
      // 1. 确定要使用的工具
      const selectedTool = await this.determineToolForChat(dto);
      this.logger.log(`🎯 Selected tool: ${selectedTool}`);

      // 2. 执行对应的操作
      const result = await this.executeToolForChat(dto, selectedTool, req);

      const processingTime = Date.now() - startTime;
      this.logger.log(`✅ Unified chat completed in ${processingTime}ms, tool: ${selectedTool}`);
      ...
      this.logger.error(`❌ Unified chat failed: ${message}`, error as any);
  ...
  /**
   * SSE 流式 Chat 接口 - 支持实时文字流式输出
   * 适用于纯文本对话和图片分析等场景
   */
  @Post('chat-stream')
  async unifiedChatStream(
    @Body() dto: UnifiedChatDto,
    @Req() req: any,
    @Res({ passthrough: false }) res: FastifyReply,
  ): Promise<void> {
    const startTime = Date.now();
    this.logger.log('🚀 Unified chat stream request:', {
      prompt: dto.prompt ? dto.prompt.substring(0, 50) + '...' : '(empty)',
      mode: dto.mode || 'auto',
      hasImages: !!dto.attachments?.images?.length,
      imageCount: dto.attachments?.images?.length || 0,
    ...
    // 设置 SSE 响应头
    // 由于使用 @Res({ passthrough: false })，需要手动复制 CORS 头
    // 从 Fastify 请求中获取 origin，并设置 CORS 响应头
    const origin = req.raw?.headers?.origin || req.headers?.origin;
    if (origin) {
    ...
    res.raw.setHeader('X-Accel-Buffering', 'no'); // 禁用 nginx 缓冲
    res.raw.flushHeaders();

    // 发送 SSE 事件的辅助函数
    const sendEvent = (data: SSEEventData) => {
    ...
    try {
      ...
      const textModel = this.resolveTextModel(providerName, dto.model);

      this.logger.log(`🎯 Stream selected tool: ${selectedTool}`);
      ...
      // 2. 根据工具类型执行流式或非流式操作
      const images = dto.attachments?.images || [];
      let fullText = '';

      // 支持流式输出的工具
      if (selectedTool === 'chatResponse') {
        // 纯文本对话 - 流式输出
      ...
      } else if (selectedTool === 'analyzeImage') {
        // 图片分析 - 流式输出
        const sourceImage = images[0];
        if (!sourceImage) {
          throw new BadRequestException('analyzeImage requires an image');
        }

        const customApiKey = this.isGeminiProvider(providerName) ? await this.getUserCustomApiKey(req) : null;

        const analysisStream = this.imageGeneration.analyzeImageStream({
          prompt: dto.prompt,
          sourceImage,
          customApiKey,
        });
        ...
        sendEvent({
          type: 'done',
          data: { text: fullText, analysis: fullText },
          model: textModel,
          provider: dto.aiProvider || 'gemini',
        });

      } else {
        // 其他工具（图片生成、视频等）- 非流式，直接执行后返回结果
        const result = await this.executeToolForChat(dto, selectedTool, req);
      ...
      const processingTime = Date.now() - startTime;
      this.logger.log(`✅ Unified chat stream completed in ${processingTime}ms, tool: ${selectedTool}`);

    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      this.logger.error(`❌ Unified chat stream failed: ${message}`, error as any);
   ...
   * 根据请求确定要使用的工具
   */
  private async determineToolForChat(dto: UnifiedChatDto): Promise<ChatTool> {
    // 如果指定了模式，直接映射
    if (dto.mode && dto.mode !== ChatMode.AUTO) {
      const modeToToolMap: Record<ChatMode, ChatTool> = {
        [ChatMode.AUTO]: 'chatResponse',
        [ChatMode.TEXT]: 'chatResponse',
        [ChatMode.GENERATE]: 'generateImage',
        [ChatMode.EDIT]: 'editImage',
        [ChatMode.BLEND]: 'blendImages',
        [ChatMode.ANALYZE]: 'analyzeImage',
        [ChatMode.VIDEO]: 'generateVideo',
        [ChatMode.VECTOR]: 'generatePaperJS',
        [ChatMode.PDF]: 'analyzePdf',
      };
      return modeToToolMap[dto.mode];
    }

    // Auto 模式：根据附件自动判断
    const images = dto.attachments?.images || [];
    const hasPdf = !!dto.attachments?.pdf;
    ...
    const availableTools = this.sanitizeAvailableTools(undefined, allowVector);
    ...
    if (providerName) {
      try {
        ...
        if (result.success && result.data) {
          return this.enforceSelectedTool(result.data.selectedTool, availableTools) as ChatTool;
        }
      } catch (error) {
        this.logger.warn(`Provider tool selection failed, falling back to Gemini`, error as any);
      }
    }

    // 降级到 Gemini
    const result = await this.ai.runToolSelectionPrompt(dto.prompt, availableTools);
    return this.enforceSelectedTool(result.selectedTool, availableTools) as ChatTool;
  }

  /**
   * 执行选定的工具
   */
  private async executeToolForChat(
    dto: UnifiedChatDto,
    tool: ChatTool,
    req: any,
  ): Promise<{ data: any; reasoning?: string; model?: string }> {
    const providerName = dto.aiProvider && dto.aiProvider !== 'gemini' ? dto.aiProvider : null;
    const images = dto.attachments?.images || [];

    switch (tool) {
      case 'generateImage': {
        const imageModel = this.resolveImageModel(providerName, dto.model);
        const serviceType = this.getImageGenerationServiceType(imageModel, providerName || undefined);
        const customApiKey = this.isGeminiProvider(providerName) ? await this.getUserCustomApiKey(req) : null;

        const result = await this.withCredits(req, serviceType, imageModel, async () => {
          if (providerName && providerName !== 'gemini-pro') {
            ...
            const res = await provider.generateImage({
              prompt: dto.prompt,
              model: imageModel,
              imageOnly: dto.imageOptions?.imageOnly,
              aspectRatio: dto.imageOptions?.aspectRatio,
              imageSize: dto.imageOptions?.imageSize,
              thinkingLevel: dto.imageOptions?.thinkingLevel,
              outputFormat: dto.imageOptions?.outputFormat,
              providerOptions: dto.providerOptions,
            });
            if (res.success && res.data) {
              const watermarked = await this.watermarkIfNeeded(res.data.imageData, req);
              return { imageData: watermarked, textResponse: res.data.textResponse || '', metadata: res.data.metadata };
            }
            throw new Error(res.error?.message || 'Failed to generate image');
          }

          const data = await this.imageGeneration.generateImage({ ...dto.imageOptions, prompt: dto.prompt, customApiKey });
          const watermarked = await this.watermarkIfNeeded(data.imageData, req);
          return { ...data, imageData: watermarked };
        }, 0, 1, !!customApiKey);

        return {
          data: { imageData: result.imageData, text: result.textResponse, metadata: result.metadata },
          model: imageModel,
        };
      }
      ...
      case 'editImage': {
        const sourceImage = images[0];
        if (!sourceImage) {
          throw new BadRequestException('editImage requires at least one image');
        }

        const imageModel = this.resolveImageModel(providerName, dto.model);
        const customApiKey = this.isGeminiProvider(providerName) ? await this.getUserCustomApiKey(req) : null;

        const result = await this.withCredits(req, 'gemini-image-edit', imageModel, async () => {
          if (providerName && providerName !== 'gemini-pro') {
            const provider = this.factory.getProvider(dto.model, providerName);
            ...
            throw new Error(res.error?.message || 'Failed to edit image');
          }

          const data = await this.imageGeneration.editImage({ ...dto.imageOptions, prompt: dto.prompt, sourceImage, customApiKey });
          const watermarked = await this.watermarkIfNeeded(data.imageData, req);
          return { ...data, imageData: watermarked };
        }, 1, 1, !!customApiKey);
      ...
      case 'blendImages': {
        if (images.length < 2) {
          throw new BadRequestException('blendImages requires at least 2 images');
        }

        const imageModel = this.resolveImageModel(providerName, dto.model);
        const customApiKey = this.isGeminiProvider(providerName) ? await this.getUserCustomApiKey(req) : null;

        const result = await this.withCredits(req, 'gemini-image-blend', imageModel, async () => {
          if (providerName && providerName !== 'gemini-pro') {
            const provider = this.factory.getProvider(dto.model, providerName);
            const res = await provider.blendImages({
              prompt: dto.prompt,
              sourceImages: images,
              model: imageModel,
              imageOnly: dto.imageOptions?.imageOnly,
            ...
            throw new Error(res.error?.message || 'Failed to blend images');
          }

          const data = await this.imageGeneration.blendImages({ ...dto.imageOptions, prompt: dto.prompt, sourceImages: images, customApiKey });
          const watermarked = await this.watermarkIfNeeded(data.imageData, req);
          return { ...data, imageData: watermarked };
        }, images.length, 1, !!customApiKey);
      ...
      case 'analyzeImage': {
        ...
        const result = await this.withCredits(req, 'gemini-image-analyze', imageModel, async () => {
          if (providerName && providerName !== 'gemini-pro') {
            const provider = this.factory.getProvider(dto.model, providerName);
            ...
            throw new Error(res.error?.message || 'Failed to analyze image');
          }

          return this.imageGeneration.analyzeImage({ prompt: dto.prompt, sourceImage, customApiKey });
        }, 1, 0, !!customApiKey);

        return {
          data: { text: result.text, analysis: result.text },
          model: imageModel,
        };
      }
      ...
      case 'chatResponse': {
        ...
        const result = await this.withCredits(req, 'gemini-text', textModel, async () => {
          if (providerName && providerName !== 'gemini-pro') {
            const provider = this.factory.getProvider(dto.model, providerName);
            ...
            throw new Error(res.error?.message || 'Failed to generate text');
          }

          return this.imageGeneration.generateTextResponse({ prompt: dto.prompt, enableWebSearch: dto.enableWebSearch, customApiKey });
        }, undefined, undefined, !!customApiKey);

        return {
          data: { text: result.text, webSearchResult: (result as any).webSearchResult },
          model: textModel,
        };
      }

      case 'generateVideo': {
        const quality = dto.videoOptions?.quality === 'sd' ? 'sd' : 'hd';
        const serviceType: ServiceType = quality === 'sd' ? 'sora-sd' : 'sora-hd';
        const model = this.sora2VideoService.getModelForQuality(quality);
        const referenceImageUrls = dto.videoOptions?.referenceImageUrls?.filter(
          (url) => typeof url === 'string' && url.trim().length > 0,
        ) || [];

        const result = await this.withCredits(req, serviceType, model, async () => {
          const res = await this.sora2VideoService.generateVideo({
            prompt: dto.prompt,
            referenceImageUrls,
            quality,
            aspectRatio: dto.videoOptions?.aspectRatio,
            duration: dto.videoOptions?.duration,
          });
          ...
          if (isAdmin) {
            let proxiedUrl = res.videoUrl;
            try {
              const uploaded = await this.videoWatermarkService.uploadOriginalToOSS(res.videoUrl);
              proxiedUrl = uploaded.url;
            } catch {
              // fallback to raw URL
            }
            return { ...res, videoUrl: proxiedUrl };
          }

          try {
            const wm = await this.videoWatermarkService.addWatermarkAndUpload(res.videoUrl, { text: 'Tanvas AI' });
            return { ...res, videoUrl: wm.url };
          } catch {
            return res;
          }
        }, referenceImageUrls.length || undefined, 0);

        return {
          data: { videoUrl: result.videoUrl, thumbnailUrl: result.thumbnailUrl, metadata: result },
          model,
        };
      }
      ...
      case 'generatePaperJS': {
        ...
        const result = await this.withCredits(req, 'gemini-paperjs', textModel, async () => {
          if (providerName && providerName !== 'gemini-pro') {
            const provider = this.factory.getProvider(dto.model, providerName);
            const res = await provider.generatePaperJS({
              prompt: dto.prompt,
              model: textModel,
              thinkingLevel: dto.vectorOptions?.thinkingLevel,
              canvasWidth: dto.vectorOptions?.canvasWidth,
              canvasHeight: dto.vectorOptions?.canvasHeight,
            });
            if (res.success && res.data) {
              return { code: res.data.code, explanation: res.data.explanation };
            }
            throw new Error(res.error?.message || 'Failed to generate PaperJS code');
          }

          return this.imageGeneration.generatePaperJSCode({
            prompt: dto.prompt,
            model: dto.model,
            thinkingLevel: dto.vectorOptions?.thinkingLevel,
            canvasWidth: dto.vectorOptions?.canvasWidth,
        ...
        return {
          data: { code: result.code, explanation: result.explanation },
          model: textModel,
        };
      }
