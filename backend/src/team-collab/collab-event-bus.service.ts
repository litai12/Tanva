...
const KEY_PREFIX = 'canvas:project:';

function channelOf(projectId: string): string {
  return `${KEY_PREFIX}${projectId}`;
}

function projectIdFromChannel(channel: string): string | null {
  return channel.startsWith(KEY_PREFIX) ? channel.slice(KEY_PREFIX.length) : null;
}
...
export class CollabEventBus implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit(): Promise<void> {
    ...
    try {
      ...
      this.subscriber.on('message', (channel: string, raw: string) => {
        ...
        if (!pid) return;
        const handlers = this.localHandlers.get(pid);
  ...
  async publish(projectId: string, envelope: CollabEnvelope): Promise<void> {
    ...
    if (this.publisher && !this.degraded) {
      try {
        await this.publisher.publish(channelOf(projectId), json);
        return;
      } catch (err) {
        this.logger.warn(`publish failed for ${projectId}: ${(err as Error).message}`);
        this.degraded = true;
        this.degradedSince = Date.now();
      }
    }
    // Degraded fallback: deliver to in-process handlers directly.
    const handlers = this.localHandlers.get(projectId);
    if (!handlers) return;
    for (const h of handlers) {
      try {
        h(envelope);
  ...
  async subscribe(projectId: string, handler: Handler): Promise<() => void> {
    let set = this.localHandlers.get(projectId);
    if (!set) {
      set = new Set();
      this.localHandlers.set(projectId, set);
    }
    set.add(handler);
    ...
    return () => this.unsubscribe(projectId, handler);
  }

  private unsubscribe(projectId: string, handler: Handler): void {
    const set = this.localHandlers.get(projectId);
    if (!set) return;
    set.delete(handler);
    if (set.size === 0) {
      this.localHandlers.delete(projectId);
      const channel = channelOf(projectId);
      if (this.subscriber && this.subscribedChannels.has(channel)) {
