...
CREATE OR REPLACE FUNCTION build_wan_linear_pricing_uplifted(
...
CREATE OR REPLACE FUNCTION apply_wan_pricing_rules_fix(config jsonb)
RETURNS jsonb
LANGUAGE plpgsql
AS $fn$
DECLARE
...
BEGIN
  next_config := patch_managed_model_vendor(
    ...
    jsonb_build_object(
      'creditsPerCall', 400,
      'priceYuan', 4,
      'pricing', build_wan_linear_pricing_uplifted('["t2v","i2v"]'::jsonb)
    ),
    jsonb_build_object(
      'specPricing',
      jsonb_build_object(
  ...
  next_config := patch_managed_model_vendor(
    ...
    jsonb_build_object(
      'creditsPerCall', 400,
      'priceYuan', 4,
      'pricing', build_wan_linear_pricing_uplifted('["r2v"]'::jsonb)
    ),
    jsonb_build_object(
      'specPricing',
      jsonb_build_object(
  ...
  next_config := patch_managed_model_vendor(
    ...
    jsonb_build_object(
      'creditsPerCall', 400,
      'priceYuan', 4,
      'pricing', build_wan_linear_pricing_uplifted('["i2v"]'::jsonb)
    ),
    jsonb_build_object(
      'specPricing',
      jsonb_build_object(
...
SET "value" = apply_wan_pricing_rules_fix(COALESCE("value"::jsonb, '{}'::jsonb))::text
WHERE "key" = 'model_provider_mapping_v2';

DROP FUNCTION IF EXISTS apply_wan_pricing_rules_fix(jsonb);
DROP FUNCTION IF EXISTS build_wan_linear_pricing_uplifted(jsonb);
