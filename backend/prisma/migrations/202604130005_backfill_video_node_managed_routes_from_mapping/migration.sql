...
FROM setting,
LATERAL build_node_managed_payload(setting.cfg, 'seedance-1.5') AS payload
WHERE n."nodeKey" = 'doubaoVideo'
...
FROM setting,
LATERAL build_node_managed_payload(setting.cfg, 'seedance-2.0') AS payload
WHERE n."nodeKey" = 'seedance20Video'
  AND payload IS NOT NULL;
...
FROM setting,
LATERAL build_node_managed_payload(setting.cfg, 'vidu-q2') AS payload
WHERE n."nodeKey" = 'viduVideo'
  AND payload IS NOT NULL;
...
FROM setting,
LATERAL build_node_managed_payload(setting.cfg, 'vidu-q3') AS payload
WHERE n."nodeKey" = 'viduQ3'
  AND payload IS NOT NULL;
...
FROM setting,
LATERAL build_node_managed_payload(setting.cfg, 'kling-o3') AS payload
WHERE n."nodeKey" = 'klingO1Video'
  AND payload IS NOT NULL;
