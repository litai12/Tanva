...
func normalizeRequestedModelName(modelName string) (original string, routing string) {
	original = strings.TrimSpace(modelName)
	if original == "" {
		return "", ""
	}
	routing = model.CanonicalModelKey(original)
	if routing == "" {
		routing = original
	}
	...
	return func(c *gin.Context) {
		...
		if ok {
			...
			if shouldSelectChannel {
				...
				if preferredChannelID, found := service.GetPreferredChannelByAffinity(c, routingModel, usingGroup); found {
					...
					if err == nil && preferred != nil {
						if preferred.Status != common.ChannelStatusEnabled {
							...
							for _, g := range autoGroups {
								if model.IsChannelEnabledForGroupModel(g, routingModel, preferred.Id) {
									selectGroup = g
						...
						} else if model.IsChannelEnabledForGroupModel(usingGroup, routingModel, preferred.Id) {
							channel = preferred
				...
				if channel == nil {
					...
					if err != nil {
						...
						message := i18n.T(c, i18n.MsgDistributorGetChannelFailed, map[string]any{"Group": showGroup, "Model": displayModel, "Error": err.Error()})
