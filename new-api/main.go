...
func main() {
	...
	indexPage, err = resolveIndexPage(indexPage)
	if err != nil {
		common.FatalLog("failed to resolve web index page: " + err.Error())
		return
	}
	...
	// 设置路由
	err = router.SetRouter(server, buildFS, indexPage)
	if err != nil {
...
func resolveIndexPage(defaultIndexPage []byte) ([]byte, error) {
	webDistDir := strings.TrimSpace(os.Getenv("WEB_DIST_DIR"))
	if webDistDir == "" {
		return defaultIndexPage, nil
	}

	indexPath := filepath.Join(webDistDir, "index.html")
	indexBytes, err := os.ReadFile(indexPath)
	if err != nil {
		return nil, fmt.Errorf("read WEB_DIST_DIR index %s: %w", indexPath, err)
	}
