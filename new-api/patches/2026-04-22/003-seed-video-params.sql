-- 003-seed-video-params.sql
-- Purpose: seed kind + params_def for video models.
-- Source: ModelCapabilityCatalog.videoOptions (converted to unified params format).
-- The params_def JSON is the exact inverse of the hono-api adapter functions,
-- guaranteeing a lossless round-trip through convertParamsToVideoOptions().
...
WHERE model_name = 'kling-v3' AND deleted_at IS NULL;
...
UPDATE models SET
  ...
  updated_time = EXTRACT(EPOCH FROM NOW())::bigint
WHERE model_name = 'veo3.1-pro' AND deleted_at IS NULL;

-- veo_3_1 (alias for veo3.1-pro, same capabilities)
UPDATE models SET
  kind         = 'video',
  capabilities = '[]',
  params_def   = $json$[
  ...
  updated_time = EXTRACT(EPOCH FROM NOW())::bigint
WHERE model_name = 'veo_3_1' AND deleted_at IS NULL;
...
UPDATE models SET
  ...
  updated_time = EXTRACT(EPOCH FROM NOW())::bigint
WHERE model_name = 'veo_3_1-fast' AND deleted_at IS NULL;

-- Seedance 2.0  (upstream: doubao-seedance-2-0-260128)
UPDATE models SET
  kind         = 'video',
  capabilities = '[]',
  params_def   = $json$[
...
-- Seedance 2.0 Fast  (upstream: doubao-seedance-2-0-fast-260128)
UPDATE models SET
  kind         = 'video',
  capabilities = '[]',
  params_def   = $json$[
  ...
  updated_time = EXTRACT(EPOCH FROM NOW())::bigint
WHERE model_name = 'Seedance 2.0 Fast' AND deleted_at IS NULL;
