		...
		for _, input := range inputs {
			...
			if common.GetJsonType(input.Content) == "array" {
				...
				for _, itemAny := range array {
					...
					case "input_video":
						// video_url may be string or object with url field
						var videoUrl string
						switch v := item["video_url"].(type) {
						case string:
							videoUrl = v
						case map[string]any:
							if url, ok := v["url"].(string); ok {
						...
						mediaInputs = append(mediaInputs, MediaInput{Type: "input_video", VideoUrl: videoUrl})
					}
				}
			}
		}
