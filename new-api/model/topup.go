...
// RechargeWxPay marks a WeChat Pay Native order as paid and credits the user quota.
func RechargeWxPay(tradeNo string, callerIp string) (err error) {
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		...
		if topUp.PaymentMethod != "wxnative" {
			return ErrPaymentMethodMismatch
		}
		if topUp.Status == common.TopUpStatusSuccess {
			return nil
	...
	if quotaToAdd > 0 {
		RecordTopupLog(topUp.UserId, fmt.Sprintf("微信支付充值成功，充值额度: %v，支付金额: %.2f", logger.FormatQuota(quotaToAdd), topUp.Money), callerIp, topUp.PaymentMethod, "wxnative")
	}
	return nil
}
