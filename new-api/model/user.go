...
func (user *User) Insert(inviterId int) error {
	...
	createDefaultTokenForUser(user.Id, user.Username)
	return nil
}
...
// createDefaultTokenForUser 为新注册用户生成一个永不过期、无限额度的默认令牌。
// 受 constant.GenerateDefaultToken 控制，所有注册路径（邮箱/手机号/OAuth/微信/管理员创建）统一在此处生成。
// 任何失败仅记录日志，不阻断注册流程。
func createDefaultTokenForUser(userId int, username string) {
