...
type Properties struct {
	...
	// Spec — surfaced in task log so debuggers (and 3rd-party integrators) can
...
type TaskPrivateData struct {
	...
	VolcGroupID    string              `json:"volc_group_id,omitempty"`   // Volcengine asset group ID for cleanup after task completion
}
...
func DeleteOldTasks(ctx context.Context, targetTimestamp int64, limit int) (int64, error) {
	...
	for {
		...
		result := DB.Where("created_at < ?", targetTimestamp).Limit(limit).Delete(&Task{})
		if result.Error != nil {
