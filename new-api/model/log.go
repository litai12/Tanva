...
var financialLogTypes = []int{LogTypeTopup, LogTypeConsume, LogTypeRefund}
...
// GetPublicModelStats 查询最近 24 小时各模型调用次数与成功次数，最多返回 20 条
func GetPublicModelStats() ([]PublicModelStat, error) {
	since := time.Now().Add(-24 * time.Hour).Unix()
	var stats []PublicModelStat
	err := LOG_DB.Table("logs").
		Select("model_name, COUNT(*) AS call_count, SUM(CASE WHEN type = ? THEN 1 ELSE 0 END) AS success_count", LogTypeConsume).
