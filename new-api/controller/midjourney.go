...
func GetAllMidjourney(c *gin.Context) {
	...
	if setting.MjForwardUrlEnabled {
		for i, midjourney := range items {
			// Use stored CDN URL when available; fall back to proxy only when empty.
			if midjourney.ImageUrl == "" {
				midjourney.ImageUrl = system_setting.ServerAddress + "/mj/image/" + midjourney.MjId
			}
			sanitizeMidjourneyListItem(midjourney)
			items[i] = midjourney
		}
	} else {
...
func sanitizeMidjourneyListItem(midjourney *model.Midjourney) {
	if midjourney == nil {
		return
	}
	midjourney.Prompt = model.SanitizeLargeTextForLog(midjourney.Prompt)
	midjourney.PromptEn = model.SanitizeLargeTextForLog(midjourney.PromptEn)
	midjourney.FailReason = model.SanitizeLargeTextForLog(midjourney.FailReason)
	midjourney.Properties = model.SanitizeLargeTextForLog(midjourney.Properties)
	midjourney.Buttons = model.SanitizeLargeTextForLog(midjourney.Buttons)
}
