...
// ProxyArkSeed3D forwards all /proxy/ark/* requests to the ark-seed3d channel
// (ark.cn-beijing.volces.com/api/v3), appending the captured path.
func ProxyArkSeed3D(c *gin.Context) {
	ch, err := getChannelByName("ark-seed3d")
	if err != nil {
		c.JSON(http.StatusServiceUnavailable, gin.H{"error": "no ark-seed3d channel: " + err.Error()})
		return
	}
