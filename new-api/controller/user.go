...
type BatchQuotaRequest struct {
	Mode  string `json:"mode"`  // "add" | "subtract"
	Value int    `json:"value"` // Quota 单位，必须 > 0
}

// BatchAdjustUserQuota admin 批量增减全体启用用户的余额
func BatchAdjustUserQuota(c *gin.Context) {
	...
	if req.Mode == "add" {
		res := model.DB.Exec(
			"UPDATE users SET quota = quota + ? WHERE status = 1 AND deleted_at IS NULL",
			req.Value,
		)
		if res.Error != nil {
		...
		res := model.DB.Exec(
			"UPDATE users SET quota = GREATEST(quota - ?, 0) WHERE status = 1 AND deleted_at IS NULL",
			req.Value,
		)
		if res.Error != nil {
			common.ApiError(c, res.Error)
	...
	model.RecordLog(0, model.LogTypeSystem, fmt.Sprintf(
		"管理员 %s(id=%d) 对全体启用用户批量%s额度 %s，影响 %d 个用户",
		adminName, adminId, modeText, logger.LogQuota(req.Value), affected,
	))
