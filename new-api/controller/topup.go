...
func GetTopUpInfo(c *gin.Context) {
	...
	// 如果启用了微信原生支付，添加到支付方法列表
	enableWxNative := setting.WxPayEnabled()
	if enableWxNative {
		hasWxNative := false
		for _, method := range payMethods {
			if method["type"] == "wxnative" {
				hasWxNative = true
				break
			}
		}
	...
	data := gin.H{
		...
		"enable_wxnative_topup": enableWxNative,
		...
		"wxnative_min_topup": setting.WxPayMinTopUp,
