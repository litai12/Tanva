...
func TestShouldRetryByStatusCode(t *testing.T) {
	...
	require.False(t, ShouldRetryByStatusCode(504)) // 504 still in alwaysSkip; overrides the 500-599 range
	require.True(t, ShouldRetryByStatusCode(524))  // 524 removed from alwaysSkip; now retries per 500-599 range
...
func TestShouldRetryByStatusCode_DefaultMatchesLegacyBehavior(t *testing.T) {
	...
	require.True(t, ShouldRetryByStatusCode(524))  // 524 now retries: try another channel on Cloudflare timeout
...
func TestIsAlwaysSkipRetryStatusCode(t *testing.T) {
	...
	require.False(t, IsAlwaysSkipRetryStatusCode(524)) // 524 removed from always-skip: retries on another channel
