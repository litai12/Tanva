...
// retry for 1xx, 3xx, 4xx(except 400/408), 5xx(except 504), and no retry for 2xx.
// 524 (Cloudflare origin timeout) is included so the relay retries on a different channel.
...
var alwaysSkipRetryStatusCodes = map[int]struct{}{
	504: {},
	// 524 removed: Cloudflare origin timeout is channel-specific and should
	// trigger a retry on a different channel rather than failing immediately.
}
