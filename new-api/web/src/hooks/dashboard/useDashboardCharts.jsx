...
export const useDashboardCharts = (
  ...
  // ========== Admin: 渠道成功率 ==========
  const [spec_success_rate, setSpecSuccessRate] = useState({
    type: 'bar',
    data: [{ id: 'successRateData', values: [] }],
    xField: 'model',
    yField: 'successRate',
    seriesField: 'channel',
    stack: false,
        ...
        content: [
          {
            ...
            value: (datum) =>
              `${(datum['successRate'] || 0).toFixed(2)}% (${datum['total'] || 0}次)`,
          },
        ],
      },
    },
  ...
  const updateSuccessRateChart = useCallback(
    (data) => {
      const values = data.map((item) => ({
        model: item.model_name,
        channel: item.channel_name,
        successRate: parseFloat(((item.success_rate || 0) * 100).toFixed(2)),
        total: item.total,
      }));

      const totalCalls = data.reduce((sum, item) => sum + (item.total || 0), 0);

      setSpecSuccessRate((prev) => ({
        ...prev,
        data: [{ id: 'successRateData', values }],
        title: {
          ...prev.title,
          subtext: `${t('总调用')}：${renderNumber(totalCalls)}`,
        },
      }));
    },
    [t],
