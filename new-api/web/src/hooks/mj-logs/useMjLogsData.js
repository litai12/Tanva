...
export const useMjLogsData = () => {
  ...
  const getDefaultColumnVisibility = () => {
    return {
      ...
      [COLUMN_KEYS.UPSTREAM_REQUEST]: true,
      [COLUMN_KEYS.REF_IMAGES]: true,
    };
  };
  ...
  const loadLogs = async (page = 1, size = pageSize) => {
    setLoading(true);
    const { channel_id, user_id, mj_id, start_timestamp, end_timestamp } =
      getFormValues();
    let localStartTimestamp = Date.parse(start_timestamp);
    let localEndTimestamp = Date.parse(end_timestamp);
    const url = isAdminUser
      ? `/api/mj/?p=${page}&page_size=${size}&channel_id=${channel_id}&user_id=${user_id}&mj_id=${mj_id}&start_timestamp=${localStartTimestamp}&end_timestamp=${localEndTimestamp}`
