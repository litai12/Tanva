...
import { buildCurlExample, getModelKind } from '../../helpers/modelKind';
...
export const useModelPricingData = () => {
  ...
  const copyCurl = async (model) => {
    const curl = buildCurlExample(model);
    if (await copy(curl)) {
      showSuccess(t('已复制 cURL 示例'));
    } else {
      Modal.error({ title: t('无法复制到剪贴板，请手动复制'), content: curl });
    }
  };
