...
import RequestTraceEntry from '../../components/table/usage-logs/components/RequestTraceEntry';
...
export const useLogsData = () => {
  ...
  const [conversationGroups, setConversationGroups] = useState({}); // key: conversation_id, value: { loading, logs }
  ...
  const getFormValues = () => {
    // 当 formApi 未就绪时（初次加载），回退到 URL 参数
  ...
  const openRequestTraceModal = async (log) => {
    if (!log?.request_id || loadingRequestTrace) {
      return;
    }
    ...
    const url = `${path}?request_id=${encodeURIComponent(log.request_id)}`;
    try {
  ...
  // Fetch logs belonging to the same conversation
  const fetchConversationGroup = async (conversationId) => {
    if (!conversationId || conversationGroups[conversationId]?.logs) return;
    setConversationGroups(prev => ({ ...prev, [conversationId]: { loading: true, logs: [] } }));
    try {
      const endpoint = isAdminUser ? '/api/log' : '/api/user/log';
      const res = await API.get(`${endpoint}?type=2&conversation_id=${encodeURIComponent(conversationId)}&p=1&page_size=50`);
      const items = res?.data?.data?.items ?? [];
      setConversationGroups(prev => ({ ...prev, [conversationId]: { loading: false, logs: items } }));
    } catch {
      setConversationGroups(prev => ({ ...prev, [conversationId]: { loading: false, logs: [] } }));
    }
  };
  ...
  const toggleConversationExpand = (conversationId) => {
    setExpandedConvIds(prev => {
      ...
      if (next.has(conversationId)) {
        ...
        fetchConversationGroup(conversationId);
      }
      return next;
    });
  };
  ...
  const setLogsFormat = (logs) => {
    ...
    // 按 conversation_id 分组：同会话的调用合并为一行
    const seenConvIds = new Set();
    const groupedResult = [];
    for (const log of logs) {
      if (!log.conversation_id) {
        groupedResult.push(log);
      } else if (!seenConvIds.has(log.conversation_id)) {
        seenConvIds.add(log.conversation_id);
        log._siblings = [];
        groupedResult.push(log);
      } else {
  ...
  const hasExpandableRows = () => {
    const displayLogs = groupedLogs.length > 0 ? groupedLogs : logs;
    return displayLogs.some(
      (log) =>
        (expandData[log.key] && expandData[log.key].length > 0) ||
        (log._siblings && log._siblings.length > 0),
    );
  };
