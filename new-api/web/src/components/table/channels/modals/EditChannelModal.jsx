...
function type2secretPrompt(type) {
  ...
  switch (type) {
    ...
    case 50:
      return '按照如下格式输入: AccessKey|SecretKey, 如果上游是tanvasMart，则直接输ApiKey';
    case 51:
  ...
  const handleInputChange = (name, value) => {
    ...
    if (name === 'base_url' && value.endsWith('/v1')) {
      Modal.confirm({
        title: '警告',
        content:
          '不需要在末尾加/v1，tanvasMart会自动处理，添加后可能导致请求失败，是否继续？',
        onOk: () => {
                      ...
                      {inputs.type === 8 && (
                        <>
                          <Banner
                            type='warning'
                            description={t(
                              '如果你对接的是上游One API或者tanvasMart等转发项目，请使用OpenAI类型，不要使用此类型，除非你知道你在做什么。',
                            )}
                            className='!rounded-lg'
                          />
                          <div>
