...
export const getTaskLogsColumns = ({
  ...
  return [
    ...
    {
      key: COLUMN_KEYS.SPEC,
      title: t('规格'),
      dataIndex: 'properties',
      render: (props, record) => {
        const p = props || {};
        const tags = [];
        if (p.size) tags.push({ label: p.size, color: 'blue' });
        if (p.resolution) tags.push({ label: p.resolution, color: 'cyan' });
        if (p.aspect_ratio)
          tags.push({ label: p.aspect_ratio, color: 'teal' });
        if (p.duration)
          tags.push({ label: `${p.duration}s`, color: 'violet' });
        else if (p.seconds)
          tags.push({ label: `${p.seconds}s`, color: 'violet' });
        if (p.mode) tags.push({ label: p.mode, color: 'grey' });
        if (p.reference_count)
          tags.push({ label: `ref×${p.reference_count}`, color: 'orange' });
        else if (p.has_input_reference)
          tags.push({ label: 'ref', color: 'orange' });
        if (tags.length === 0) return <Typography.Text>-</Typography.Text>;
        return (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
            {tags.map((tag, i) => (
              <Tag key={i} color={tag.color} shape='circle' size='small'>
                {tag.label}
              </Tag>
            ))}
          </div>
