...
// Parse the raw properties JSON string from model.Midjourney.
  ...
  return [
    ...
    },
    {
      ...
      render: (text) => {
        ...
        if (!props?.upstreamRequest) return t('无');
        return (
          <Button
            size='small'
            onClick={() => openContentModal(props.upstreamRequest)}
          >
            {t('查看请求')}
          </Button>
        );
    ...
    {
      key: COLUMN_KEYS.REF_IMAGES,
      title: t('参考图'),
      dataIndex: 'properties',
      fixed: 'right',
      render: (text) => {
        const props = parseProps(text);
        if (!props?.refImages?.length) return t('无');
        return (
