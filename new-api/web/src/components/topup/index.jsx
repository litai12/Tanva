...
import WxPayQRModal from './modals/WxPayQRModal';
...
const TopUp = () => {
  ...
  // 微信原生支付相关状态
  const [enableWxNativeTopUp, setEnableWxNativeTopUp] = useState(false);
  const [wxNativeMinTopUp, setWxNativeMinTopUp] = useState(1);
  const [wxPayQROpen, setWxPayQROpen] = useState(false);
  const [wxPayCodeUrl, setWxPayCodeUrl] = useState('');
  const [wxPayTradeNo, setWxPayTradeNo] = useState('');
  ...
  const wxNativeTopUp = async () => {
    if (topUpCount < wxNativeMinTopUp) {
      showError(t('充值数量不能小于') + wxNativeMinTopUp);
      return;
    }
    setPaymentLoading(true);
    try {
      const res = await API.post('/api/user/wxpay/native', {
        amount: parseInt(topUpCount),
      });
      if (res !== undefined) {
        const { message, code_url, trade_no } = res.data;
        if (message === 'success' && code_url) {
          setWxPayCodeUrl(code_url);
          setWxPayTradeNo(trade_no);
          setWxPayQROpen(true);
        } else {
          showError(res.data?.data || t('创建微信支付订单失败'));
        }
      }
    } catch (e) {
      showError(t('支付请求失败'));
    ...
    try {
      ...
      if (success) {
        ...
        try {
          ...
          const enableWxNativeTopUp = data.enable_wxnative_topup || false;
          setEnableWxNativeTopUp(enableWxNativeTopUp);
          setWxNativeMinTopUp(data.wxnative_min_topup || 1);
    ...
    <div className='w-full max-w-7xl mx-auto relative min-h-screen lg:min-h-0 mt-[60px] px-2'>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
        <RechargeCard
          ...
          enableWxNativeTopUp={enableWxNativeTopUp}
          wxNativeTopUp={wxNativeTopUp}
