	...
	for _, message := range textRequest.Messages {
		...
		for _, part := range openaiContent {
			if part.Type == dto.ContentTypeText {
				...
				// Video URLs are passed directly as fileUri — Gemini handles them natively
				// without downloading. Base64-encoding a large video would exceed size limits.
				if source.IsURL() {
					if mimeType := getMimeTypeFromURL(source.GetIdentifier()); strings.HasPrefix(mimeType, "video/") {
						if _, ok := geminiSupportedMimeTypes[mimeType]; ok {
							parts = append(parts, dto.GeminiPart{
								FileData: &dto.GeminiFileData{
									MimeType: mimeType,
									FileUri:  source.GetIdentifier(),
								},
							})
							continue
						}
...
// getMimeTypeFromURL infers MIME type from URL file extension (ignores query params).
func getMimeTypeFromURL(urlStr string) string {
	...
	if idx := strings.LastIndex(path, "."); idx != -1 {
		return service.GetMimeTypeByExtension(path[idx+1:])
	}
	return ""
}
...
func GeminiChatHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	if common.DebugEnabled {
		logger.LogDebug(c, fmt.Sprintf("image upstream response body: %s", string(responseBody)))
	}
	var geminiResponse dto.GeminiChatResponse
