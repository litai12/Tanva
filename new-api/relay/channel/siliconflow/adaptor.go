...
func (a *Adaptor) ConvertImageRequest(c *gin.Context, info *relaycommon.RelayInfo, request dto.ImageRequest) (any, error) {
	...
	referenceImages := imageutil.ExtractReferenceImages(&request)
	if len(referenceImages) > 0 {
		sfRequest.Image = referenceImages[0]
	}
	if len(referenceImages) > 1 {
		sfRequest.Image2 = referenceImages[1]
	}
	if len(referenceImages) > 2 {
		sfRequest.Image3 = referenceImages[2]
	}
