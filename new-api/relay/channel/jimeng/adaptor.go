...
func (a *Adaptor) ConvertImageRequest(c *gin.Context, info *relaycommon.RelayInfo, request dto.ImageRequest) (any, error) {
	...
	if len(request.Extra) > 0 {
		extraData, err := json.Marshal(request.Extra)
		if err == nil {
			if err := json.Unmarshal(extraData, &payload); err != nil {
				return nil, fmt.Errorf("failed to unmarshal extra payload fields: %w", err)
			}
		}
	}
	if refs := imageutil.ExtractReferenceImages(&request); len(refs) > 0 {
