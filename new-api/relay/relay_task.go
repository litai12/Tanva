...
func RelayTaskSubmit(c *gin.Context, info *relaycommon.RelayInfo) (*TaskSubmitResult, *dto.TaskError) {
	...
	if checkedEstimator, ok := adaptor.(channel.CheckedTaskBillingEstimator); ok {
		var billingErr *dto.TaskError
		estimatedRatios, billingErr = checkedEstimator.EstimateBillingChecked(c, info)
		if billingErr != nil {
	...
	// 缓冲上游请求体字节，用于 trace 记录（不影响后续读取）
	var upstreamReqBodyStr string
	if requestBody != nil {
		upstreamReqBytes, readErr := io.ReadAll(requestBody)
		if readErr == nil {
			upstreamReqBodyStr = string(upstreamReqBytes)
			requestBody = bytes.NewReader(upstreamReqBytes)
		}
	}

	// 获取上游 URL（trace 元数据）
	upstreamURL, _ := adaptor.BuildRequestURL(info)
	...
	if err != nil {
		upsertRequestTraceAttempt(c, info, model.RequestTraceAttemptPatch{
			...
			UpstreamRequestBody: upstreamReqBodyStr,
	...
	if resp != nil && resp.StatusCode != http.StatusOK {
		...
		upsertRequestTraceAttempt(c, info, model.RequestTraceAttemptPatch{
			UpstreamURL:          upstreamURL,
			UpstreamRequestBody:  upstreamReqBodyStr,
			UpstreamResponseBody: string(responseBody),
	...
	upsertRequestTraceAttempt(c, info, model.RequestTraceAttemptPatch{
		UpstreamURL:          upstreamURL,
		UpstreamRequestBody:  upstreamReqBodyStr,
		UpstreamResponseBody: string(taskData),
	})
	if taskErr != nil {
		return nil, taskErr
	}
